...
import { aiVoiceTurnService, AiVoiceTurnRequest } from "../Services/Ai/AiVoiceTurnService.js"
...
/**
 * One hold-to-talk voice turn: transcribe, run the chat brain, speak the
 * answer. JSON in, JSON out (audio as base64). Same URL-derivation contract
 * as chat — clients swap graphql__request__api for ai__request__voice.
 */
export const ai__request__voice = onRequest({ cors: true }, async (request, response) => {
    if (request.method !== "POST") {
        response.status(405).json({ error: { message: "POST an AiVoiceTurnRequest to this endpoint." } })
        return
    }
    const turnRequest = parseVoiceTurnRequest(request.body)
    if (turnRequest === undefined) {
        response.status(400).json({
            error: { message: "The request needs an id, and audio or a transcript." },
        })
        return
    }
    ...
    try {
        const result = await aiVoiceTurnService.runTurn(turnRequest)
...
function parseVoiceTurnRequest(body: unknown): AiVoiceTurnRequest | undefined {
    ...
    const transcript =
        typeof candidate.transcript === "string" && candidate.transcript.trim() !== ""
            ...
            : undefined
    const audioBase64 =
        typeof candidate.audioBase64 === "string" && candidate.audioBase64 !== ""
            ? candidate.audioBase64
            : undefined
    if (transcript === undefined && audioBase64 === undefined) {
        return undefined
    }
    return {
        id: candidate.id,
