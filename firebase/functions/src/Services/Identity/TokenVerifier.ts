...
function verifiedTokenFromClaims(payload: JWTPayload): VerifiedToken {
    ...
    // Purpose-scoped tokens (e.g. the MFA challenge JWT — see
    // BuiltinAuthService) are never access tokens: they grant exactly the
    // endpoint that checks their purpose, nothing else.
    if (payload.purpose !== undefined) {
        throw new RpcError("UNAUTHENTICATED", "Token is not an access token.")
    }
