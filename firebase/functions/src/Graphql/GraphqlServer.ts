...
import { storageConfig } from "../Services/Storage/StorageConfig.js"
import { storageService } from "../Services/Storage/StorageService.js"
import { validatedEnv } from "../Utils/Env.js"
import { orderedBatchLoad, orderedGroupLoad } from "../Utils/DataLoaderUtils.js"
...
export class GraphqlRequestContext {
    ...
    /** One-to-many: a project id loads its memberships ([] when none). */
    readonly projectMembershipsDataloader: DataLoader<string, ProjectMembership[]>
    /** Upload rows for domains that bind them (drive entries' files/thumbs). */
    readonly uploadDataloader: DataLoader<string, Upload>
    ...
    constructor(principal: Principal | undefined) {
        ...
        this.projectMembershipsDataloader = orderedGroupLoad((projectIds) =>
            projectMembershipService.orderedBatchLoadMembershipsByProjectIds(projectIds),
        )
        this.uploadDataloader = orderedBatchLoad((ids) => storageService.orderedBatchLoadUploadsByIds(ids))
    }
}
 ...
 *
 * Introspection, the Shop catalog, the payments kernel's one-off checkout
 * surface, and the Ai voice broker are public. One-off checkout is anonymous
 * by design (buyers never sign in; see docs/payments.md) — its service
 * methods are safe without a principal because prices are server-side and
 * test completion refuses outside PAYMENTS_MODE=local. The purchases ledger
 * query and every subscription operation (createSubscriptionCheckoutSession,
 * mySubscription, createBillingPortalSession, cancelTestSubscription) are
 * deliberately NOT public — subscription checkout is never anonymous.
 * createAiVoiceSession is likewise safe: the OpenAI key stays server-side
 * and only a short-lived Realtime client secret is minted. To expose another
 * public root field, add it to the matching set below.
 */
const publicQueryRootFields = new Set<string>([
    "__schema",
    "__type",
    "shopProduct",
    "shopProducts",
...
export function buildGraphqlExpressApp(): Express {
    ...
    expressApp.use(
        ...
        // writeFile carries file bytes base64 inside the JSON body (~4/3
        // inflation plus the operation document), so the limit derives from
        // the storage kernel's cap instead of the 100kb body-parser default.
        express.json({ limit: Math.ceil(storageConfig.maxSizeBytes * (4 / 3)) + 1024 * 1024 }),
