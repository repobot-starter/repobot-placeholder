 ...
 * links to. Email-keyed methods (OTP, password) and OAuth providers (see
 * OAuthProviders.ts) resolve to the same identity when they share a verified
 * email; anonymous (guest) identities have no email.
 */
export const authIdentitiesTable = baseTable(
    "auth_identities",
    {
        ...
        // Each OAuth provider's stable subject, when the identity linked via
        // that provider (see OAuthProviders.ts for the registry).
        googleSubject: text("google_subject"),
        appleSubject: text("apple_subject"),
        githubSubject: text("github_subject"),
        facebookSubject: text("facebook_subject"),
        discordSubject: text("discord_subject"),
        xSubject: text("x_subject"),
        linkedinSubject: text("linkedin_subject"),
    ...
    (table) => [
        ...
        unique("auth_identities_apple_subject_unique").on(table.appleSubject),
        unique("auth_identities_github_subject_unique").on(table.githubSubject),
        unique("auth_identities_facebook_subject_unique").on(table.facebookSubject),
        unique("auth_identities_discord_subject_unique").on(table.discordSubject),
        unique("auth_identities_x_subject_unique").on(table.xSubject),
        unique("auth_identities_linkedin_subject_unique").on(table.linkedinSubject),
