...
export const usersTable = baseTable(
    "users",
    {
        ...
        // References uploads.id by convention only (storage kernel). Set by
        // the Settings avatar flow; null until the user picks an avatar.
...
export const userUpdateSchema = userInsertSchema
    .pick({ displayName: true, status: true, avatarUploadId: true })
    .partial()
