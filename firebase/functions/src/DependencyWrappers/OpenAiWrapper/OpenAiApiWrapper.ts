...
/**
 * Where the wrapper sends its requests. The Responses-API protocol is
 * identical in both real modes; the endpoint is the only difference:
 *
 * - AI_MODE=openai:  api.openai.com with the account's own OPENAI_API_KEY;
 *   realtime client secrets mint at /realtime/client_secrets.
 * - AI_MODE=gateway: the platform's AI gateway (AI_GATEWAY_URL) with the
 *   per-environment AI_GATEWAY_TOKEN — keyless deploys, billed to the
 *   account's platform credits; the gateway's realtime mint route is
 *   /realtime/sessions.
 */
interface OpenAiEndpoint {
    baseUrl: string
    bearerToken: string
    realtimeSecretsPath: string
}

function resolveEndpoint(): OpenAiEndpoint {
    const env = validatedEnv()
    if (env.AI_MODE === "gateway") {
        const url = env.AI_GATEWAY_URL
        const token = env.AI_GATEWAY_TOKEN
        if (url === undefined || url === "" || token === undefined || token === "") {
            throw new RpcError(
                "FAILED_PRECONDITION",
                "AI_MODE=gateway needs AI_GATEWAY_URL and AI_GATEWAY_TOKEN (injected by the " +
                    "platform on deploy). Redeploy the environment, or run locally with " +
                    "AI_MODE=local (simulated assistant).",
            )
        }
        return {
            baseUrl: url.replace(/\/+$/, ""),
            bearerToken: token,
            realtimeSecretsPath: "/realtime/sessions",
        }
    }
    ...
    return {
        ...
        bearerToken: apiKey,
        realtimeSecretsPath: "/realtime/client_secrets",
    }
}

/**
 * The real OpenAI client, used when AI_MODE=openai (the account's own key
 * against api.openai.com) and AI_MODE=gateway (the platform's AI gateway
 * with a per-environment bearer token — same protocol, different endpoint;
 * see resolveEndpoint). Talks to the Responses API (streaming SSE) and the
 * Realtime client-secrets endpoint over raw fetch, mirroring the Stripe
 * wrapper's no-SDK approach. Credentials are injected by the platform at
 * deploy time when the deploy manifest declares the AI capability; locally
 * an OpenAI key can be set in firebase/functions/.env.local.
 */
export class OpenAiApiWrapper implements OpenAiWrapper {
    ...
    async streamModelTurn(
        ...
        const response = await fetch(`${this.endpoint.baseUrl}/responses`, {
            method: "POST",
            headers: {
                authorization: `Bearer ${this.endpoint.bearerToken}`,
    ...
    async createEmbeddings(request: OpenAiEmbeddingsRequest): Promise<OpenAiEmbeddingsResult> {
        // The same endpoint split as chat: api.openai.com/v1/embeddings on a
        // BYO key, or the platform gateway's /embeddings route (identical
        // protocol, per-environment bearer token) when AI_MODE=gateway.
        const response = await fetch(`${this.endpoint.baseUrl}/embeddings`, {
            method: "POST",
            headers: {
                authorization: `Bearer ${this.endpoint.bearerToken}`,
                "content-type": "application/json",
        ...
        if (data.length !== request.input.length) {
            throw new RpcError(
                "UNAVAILABLE",
                `OpenAI: expected ${request.input.length} embeddings, got ${data.length}.`,
            )
        }
        const embeddings: number[][] = new Array<number[]>(data.length)
        for (const [position, item] of data.entries()) {
            if (!Array.isArray(item.embedding)) {
                throw new RpcError("UNAVAILABLE", "OpenAI: an embedding entry carried no vector.")
            }
            embeddings[item.index ?? position] = item.embedding
        }
        return {
            embeddings,
    ...
    async createRealtimeClientSecret(
        ...
        const response = await fetch(`${this.endpoint.baseUrl}${this.endpoint.realtimeSecretsPath}`, {
            method: "POST",
            headers: {
                authorization: `Bearer ${this.endpoint.bearerToken}`,
                "content-type": "application/json",
