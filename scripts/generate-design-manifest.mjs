...
for (const key of existsSync(packsDir)
    ...
    if (catalog.remixOf !== undefined) {
        ...
        packs[key] = {
            ...
            // merged over the base's landing by resolveCatalog) and pin its
            // layout (`landing.pages`, which replaces the base's page seeds
            // in that merge); everything else about its design space still
            // resolves through the base's entry. Only overrides are recorded
            // — a remix without one wears the base's register and seeds,
            // read from the base's entry.
            ...(catalog.landing?.style?.preset !== undefined
                ? { stylePreset: catalog.landing.style.preset }
                : {}),
            ...(catalog.landing?.pages !== undefined ? { pageSeeds: catalog.landing.pages } : {}),
...
const manifest = {
    ...
    landing: {
        ...
        // One line of art direction per register and per treatment flag
        // (web/design-system/src/marketing/theme/presetDescriptions.ts):
        // what an agent reads to pick a register from a brief, and what a
        // remix direction can quote back to the owner.
        stylePresetDescriptions: vocabulary.stylePresetDescriptions,
        treatments: vocabulary.treatmentDescriptions,
        shell: {
