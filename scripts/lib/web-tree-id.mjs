...
const included = listing.filter((line) => {
  ...
  // Design-system drift manifests are verification metadata (read only by
  // verify-ds-pristine.mjs at publish time); they can never change what Vite
  // emits, so refreshing them must not force a template rebake.
  if (path.endsWith("/.pristine-manifest.json")) return false
