...
/**
 * A description record that must cover every member of `names` — the
 * vocabulary an agent picks from is only usable if each entry says what
 * it looks like.
 */
function describedRecord(record, names, what) {
    const described = required(record, `${what} descriptions`)
    const missing = names.filter((name) => typeof described[name] !== "string" || described[name].length === 0)
    if (missing.length > 0) {
        throw new Error(`design vocabulary: ${what} without a description: ${missing.join(", ")}`)
    }
    return Object.fromEntries(names.map((name) => [name, described[name]]))
}
...
export function parseDesignVocabulary(root) {
    ...
    const descriptions = read("web/design-system/src/marketing/theme/presetDescriptions.ts")
    const presetNames = required(unionMembers(presets, "MarketingPresetName"), "style presets")
    const treatmentFlags = required(unionMembers(presets, "MarketingTreatmentFlag"), "treatment flags")
    ...
    return {
        ...
        stylePresets: presetNames,
        stylePresetAxes: required(presetAxes(presets, presetNames), "style preset axes"),
        stylePresetDescriptions: describedRecord(
            recordOfStrings(descriptions, "MARKETING_PRESET_DESCRIPTIONS"),
            presetNames,
            "style presets",
        ),
        treatmentFlags,
        treatmentDescriptions: describedRecord(
            recordOfStrings(descriptions, "MARKETING_TREATMENT_DESCRIPTIONS"),
            treatmentFlags,
            "treatment flags",
        ),
        ...
        for (const [slotPath, slot] of Object.entries(slots)) {
            ...
            if (slot.optional !== undefined) {
                if (slot.optional !== true) {
                    problems.push(`slot '${label}': "optional" must be true when present`)
                } else if (slot.kind !== "text" && slot.kind !== "media") {
                    problems.push(
                        `slot '${label}': only text and media slots may be "optional" (an empty list is already valid)`,
                    )
                }
            }
