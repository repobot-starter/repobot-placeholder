...
#
# Static-class trees run a CLIENT-ONLY stack instead: scripts/lib/stack-class.mjs
# reads the compose-stamped repobot.deploy.json and, for templates that are
# not applications (base family not app/store, no provisioned-infrastructure
# capability — the complement of the platform's isApplicationTemplate test),
# dev-up boots the web dev server alone and publishes the readiness marker
# the moment it answers. No functions emulator, no database, no
# migrations/functions build, no graphql probe: on shared-CPU sandbox pods
# the idle backend harness starved Vite and gated .dev/stack-ready minutes
# behind it (2026-09-18: a static influencer site's emulator took 5 minutes
# to spawn and the agent's readiness wait timed out twice). Checkouts
# without repobot.deploy.json (this kernel repo itself) and application
# templates keep the full harness below, unchanged.
#
...
# client|full — see the header. Everything backend below is guarded on it.
STACK_CLASS="$(node "$REPO_ROOT/scripts/lib/stack-class.mjs" "$REPO_ROOT")"
if [ "$STACK_CLASS" = "client" ]; then
    log "Static-class template (repobot.deploy.json): client-only stack — web dev server without the backend harness."
    # A static tree must not inherit a previous full-stack boot's backend
    # (e.g. a template flip from an application pack left an emulator and an
    # embedded Postgres running — exactly the shared-CPU contention this
    # mode removes). Idempotent no-ops when nothing is running.
    stop_background functions
    rm -f "$DEV_DIR/functions-port"
    if [ "${DB_MODE:-docker}" = "embedded" ]; then
        node "$REPO_ROOT/scripts/lib/embedded-pg.mjs" stop core >/dev/null 2>&1 || true
    fi
fi
...
fi # STACK_CLASS != client (db + functions port selection)
...
fi # STACK_CLASS != client (env re-pin + database)
...
# Client-only stacks have no emulator to proxy to: leaving the target unset
# makes vite serve without the /api bridge (apiProxy in web/app/vite.config.ts
# returns undefined) instead of proxying into a dead port.
if [ "$STACK_CLASS" != "client" ]; then
    WEB_ENV+=(REPOBOT_API_PROXY_TARGET="${GRAPHQL_URL%/graphql__request__api}")
fi
...
# The emulator discovers functions exactly ONCE at startup, with a hard
# timeout (default 10s). A timeout is not fatal to the process: the emulator
# stays up serving ZERO functions, forever — and being alive, every later
# dev-up's idempotency skips restarting it while the registration probe
# below 404s. That latch poisoned a whole workspace machine on 2026-09-15: a
# pool-claim template flip restarted the stack, discovery lost a 10s CPU
# race against vite's cold boot (the bundle loads in ~80ms idle), and the
# platform retried the same dead emulator for 10+ minutes. The wide timeout
# makes the transient miss effectively impossible on contended shared CPUs;
# the probe's restart-once below heals the latch if it ever happens anyway.
start_functions_emulator() {
    start_background functions env FUNCTIONS_DISCOVERY_TIMEOUT=60 "$REPO_ROOT/node_modules/.bin/firebase" emulators:start --only functions --project demo-repobot-base ${FIREBASE_CONFIG_ARGS[@]+"${FIREBASE_CONFIG_ARGS[@]}"}
}
start_functions_emulator

fi # STACK_CLASS != client (migrations + functions build + emulator start)
...
# EXISTS (any status but 404 — even a 400/500 proves registration).
#
# A persistent 404 does NOT fail the boot immediately: it is the signature
# of the discovery latch described above start_functions_emulator — an
# emulator that is alive but empty and will never load functions on its
# own. Failing here left that emulator running, so every retry of dev-up
# saw "functions already running", skipped the restart, and failed this
# same probe forever (the machine never healed without outside surgery).
# Instead, evict the poisoned emulator and start a fresh one, once; only a
# second persistent 404 — a genuinely broken bundle, not a lost timing
# race — fails the boot loudly so supervisors retry.
...
if ! probe_functions_registration; then
    warn "graphql__request__api is not registered (404) — the emulator likely lost its one-shot function discovery. Restarting it once."
    tail -n 20 "$LOG_DIR/functions.log" >&2 2>/dev/null || true
    stop_background functions
    start_functions_emulator
    wait_for_port 127.0.0.1 "$FUNCTIONS_PORT" 180 "functions emulator (after discovery-failure restart)"
    if ! { [ -f "$PID_DIR/functions.pid" ] && kill -0 "$(cat "$PID_DIR/functions.pid")" 2>/dev/null; }; then
        tail -n 40 "$LOG_DIR/functions.log" >&2 2>/dev/null || true
        fail "functions emulator process exited during the discovery-failure restart (log tail above, full log in $LOG_DIR/functions.log)."
    fi
    probe_functions_registration || fail "functions emulator is up but graphql__request__api is not registered (404) even after a fresh restart — the functions bundle itself likely fails to load; check $LOG_DIR/functions.log."
fi

fi # STACK_CLASS != client (functions readiness gates)

# Every service this class runs answered: publish readiness for agent gates
# (readyFile in repobot.sandbox.json — the platform delays agent dispatch on
# this, never the user's preview stream). Client-only stacks reach here the
# moment the web dev server answers.
