...
const SURFACES = [
    ...
    },
    {
        ...
        enforce: "kernel-only",
        customizeHint:
            "Template-shipped tests are kernel-owned and refresh-managed; in projects, " +
            "add NEW test files for app behavior and let refresh conflict on edits to " +
            "kernel-shipped tests.",
    },
    {
        dir: "packs",
        enforce: "kernel-only",
...
// .pinned-tests.json is the pinned-tests script's own bookkeeping: hashing
// it here while that script hashes this manifest made the two --write
// scripts chase each other forever (no fixpoint after any test change).
const EXCLUDED_FILES = new Set([MANIFEST_NAME, ".kernel-refresh-ref", ".pinned-tests.json"])
