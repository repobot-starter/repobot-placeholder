...
function remixProblems(key, catalog) {
    ...
    // resolveCatalog) and the platform's template flip merge a remix's
    // `landing` shallowly over the base's, keeping the base's routes (the
    // router is base code). So a remix's landing is a register override
    // (`style`) plus, optionally, a layout pin (`pages`): the page skeletons
    // it keeps when the base's layout moves on. Because the merge is shallow
    // a remix's `pages` REPLACES the base's, so a pin must seed exactly the
    // pages the base routes to, and its skeletons must be valid seeds.
    if ("landing" in catalog) {
        const overlayKeys = Object.keys(catalog.landing).filter((k) => k !== "$comment")
        const allowed = overlayKeys.every((k) => k === "style" || k === "pages")
        if (!allowed || typeof catalog.landing.style?.preset !== "string") {
            found.push(
                `remix '${key}': 'landing' may only be a register override plus an optional ` +
                    `layout pin ({ style: { preset }, pages? }, merged over the base's landing by compose) — ` +
                    `routes are inherited from the base pack`,
            )
        } else if (catalog.landing.pages !== undefined) {
            const routedPages = [...new Set(Object.values(base.landing?.routes ?? {}))].sort()
            const pinnedPages = Object.keys(catalog.landing.pages).sort()
            if (JSON.stringify(routedPages) !== JSON.stringify(pinnedPages)) {
                found.push(
                    `remix '${key}': landing.pages must pin every page the base routes ` +
                        `(${routedPages.join(", ")}) — got ${pinnedPages.join(", ")}`,
                )
            }
            for (const [pageId, page] of Object.entries(catalog.landing.pages)) {
                const sections = page?.sections
                if (!Array.isArray(sections) || sections.some((section) => typeof section?.id !== "string")) {
                    found.push(`remix '${key}': pinned page '${pageId}' needs sections[] with string ids`)
                }
            }
            const resolved = { ...base.landing, ...catalog.landing }
            for (const problem of landingSeedProblems(resolved, vocabulary)) {
                found.push(`remix '${key}': landing ${problem}`)
            }
        }
    }
    return found
