...
# Default to docker, but fall back to embedded automatically when no docker
# daemon exists (sandbox pods, minimal CI hosts). Agents kept dying on the
# "install Docker Desktop" hint in environments where that is impossible;
# the embedded binaries ship with node_modules, so just use them.
if [ -z "${DB_MODE:-}" ] && ! command -v docker >/dev/null 2>&1; then
    log "docker not found; using embedded Postgres (DB_MODE=embedded)."
    DB_MODE="embedded"
fi
...
if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
        ...
        # pgvector/pgvector:pg16 is postgres:16 plus the vector extension —
        # the retrieval kernel's migration enables it (docs/ai.md). Existing
        # containers keep working (retrieval falls back to in-app cosine);
        # recreate with --reset to get the SQL similarity path locally.
        log "Creating container $CONTAINER (pgvector/pgvector:pg16, port $PORT)..."
