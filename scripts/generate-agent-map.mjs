...
const DESIGN_VOCABULARY = parseDesignVocabulary(KERNEL_ROOT)
const LANDING_SECTION_VARIANTS = DESIGN_VOCABULARY.sectionVariants
...
/**
 * Copy-like slot paths a pack actually declares: display name, tagline,
 * headline, story. Used to tell a rename/copy ask from a managed-record
 * edit without hardcoding any one pack's vocabulary.
 */
function copySlotKeys(slots) {
    return Object.keys(slots).filter(
        (key) => /(^|\.)(name|tagline|headline|subheadline|statement)$/.test(key) || key === "siteName",
    )
}

/**
 * Short copy-vs-managed disambiguation derived from this pack's content
 * contract and managed domains. Undefined when the pack has no content
 * module. Rides near the TOP of the map (the platform caps the inlined
 * map at 6000 chars and the recipes at the tail truncate).
 */
function contentCopyGuidance(catalog) {
    const module = catalog.contentContract?.module
    if (module === undefined) return undefined
    const slots = catalog.contentContract.slots ?? {}
    const copyKeys = copySlotKeys(slots).slice(0, 4)
    const domains = Object.keys(catalog.content ?? {}).filter((key) => key !== "$comment")
    const copyHint =
        copyKeys.length > 0 ? ` (this pack: ${copyKeys.map((key) => `\`${key}\``).join(", ")})` : ""
    const managedHint =
        domains.length > 0
            ? ` Managed domains (${domains.join(", ")}) hold structured records (providers, services, hours, listings…) and have NO display-name field — a rename is never update_managed_content.`
            : " This pack has no managed-content domains; a rename is never update_managed_content."
    return {
        module,
        topNote:
            `Rename / copy: the site wordmark, taglines, and story live ONLY in \`${module}\`${copyHint}. ` +
            `Edit that module directly, then stop.${managedHint} Do not run test suites for content-module copy edits.`,
        surfacesNote:
            `The site wordmark/display name, taglines, and story copy live ONLY in the content module ` +
            `(\`${module}\`) — edit them there directly.${copyHint}${managedHint} A rename or copy ` +
            `change is a content-module edit, never update_managed_content. Do not run test suites ` +
            `for content-module copy edits; the platform gates cover regressions.`,
    }
}
...
function appCompositionSection() {
    ...
    out("   test suites for scaffolded pages, manifest/theme edits, or content-module")
    out("   copy edits; suites are only worth their minutes when you changed shared")
    out("   kernel code (web/core, web/design-system).")
    out()
...
/**
 * The register the template wears, in words: the preset's art direction and
 * what each of its treatment flags does, so an agent restyling or writing
 * copy knows the look it is working inside, and that a remix swaps it.
 * Exported for check-agent-maps, which requires it in every pack map.
 */
export function registerSection(preset) {
    if (preset === undefined || DESIGN_VOCABULARY.stylePresetDescriptions[preset] === undefined) {
        return undefined
    }
    const lines = ["### Register", ""]
    lines.push(
        `Wears the \`${preset}\` style preset (\`style.preset\`): ${DESIGN_VOCABULARY.stylePresetDescriptions[preset]}`,
    )
    const treatments = DESIGN_VOCABULARY.stylePresetAxes[preset]?.treatment ?? []
    if (treatments.length > 0) {
        lines.push("", "Its treatment flags:", "")
    }
    for (const flag of treatments) {
        lines.push(`- \`${flag}\` — ${DESIGN_VOCABULARY.treatmentDescriptions[flag]}`)
    }
    if (treatments.length > 0) {
        lines.push("")
    }
    lines.push("A remix re-values `style.preset` to any register in `docs/landing-content.md`")
    lines.push("(The registers) and keeps the content, so write copy for the section, not")
    lines.push("for the register; restyle through the preset and `repobot.theme.json`, never")
    lines.push("by hard-coding the look into content.")
    return lines.join("\n")
}
...
export function generatePackMap(repoRoot, packKey) {
    ...
    out("anywhere else.")
    const copyGuidance = contentCopyGuidance(catalog)
    if (copyGuidance !== undefined) {
        out()
        out(copyGuidance.topNote)
    }
    ...
    const register = registerSection(catalog.landing?.style?.preset)
    ...
    if (catalog.contentContract?.module !== undefined) {
        ...
        if (copyGuidance !== undefined) {
            out(`- Copy vs managed facts: ${copyGuidance.surfacesNote}`)
        }
    }
    const domains = Object.keys(catalog.content ?? {}).filter((key) => key !== "$comment")
    if (domains.length > 0) {
