...
/**
 * A pack that declares a register must describe it: the map names the
 * preset with its art direction and every treatment flag it wears (the
 * vocabulary an agent restyles within). A preset missing from the
 * description records fails vocabulary parsing before this runs.
 */
function validateRegister(subject, map, key) {
    const catalog = JSON.parse(readFileSync(path.join(repoRoot, "packs", key, "catalog.json"), "utf8"))
    const preset = catalog.landing?.style?.preset
    if (preset === undefined) {
        return
    }
    const expected = registerSection(preset)
    if (expected === undefined) {
        fail(subject, `declares style preset '${preset}', which the design vocabulary does not know`)
    } else if (!map.includes(expected)) {
        fail(subject, `map does not describe its register '${preset}' (expected the ### Register block)`)
    }
}
...
for (const key of approvedPacks) {
    ...
    validateRegister(`pack '${key}'`, map, key)
}
