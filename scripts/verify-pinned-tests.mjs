...
// Bookkeeping files are never pinned: this manifest itself, and the
// ds-pristine manifest that lives alongside the tests. Pinning the sibling
// manifest made the two --write scripts chase each other forever (each
// rewrite changed a file the other had hashed — no fixpoint, gates
// unpassable after any legitimate test change).
const UNPINNED_FILES = new Set([".pinned-tests.json", ".pristine-manifest.json"])

function* walk(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        if (entry.isDirectory()) yield* walk(path.join(dir, entry.name))
        else if (!UNPINNED_FILES.has(entry.name)) yield path.join(dir, entry.name)
    }
}
