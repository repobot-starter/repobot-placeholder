...
// A color the owner edits is content, not theme: a pack whose content
// contract declares a `color` slot (a showcase swatch's chip — the paint,
// the fabric, the product) may author `color: "#rrggbb"` in its content
// module and remix seeds. The swatch component validates the value where
// it renders; any other hex in those files is still checked.
const CONTENT_COLOR_LINE = /^\s*color: "#[0-9a-fA-F]{6}",?\s*$/
function declaresColorSlot(slots) {
    return Object.entries(slots ?? {}).some(
        ([key, slot]) =>
            (/(^|\.)color$/.test(key) && slot.kind === "text") ||
            (slot.kind === "collection" && declaresColorSlot(slot.item)),
    )
}
const catalogs = readdirSync(packsDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .flatMap((entry) => {
        try {
            return [JSON.parse(readFileSync(path.join(packsDir, entry.name, "catalog.json"), "utf8"))]
        } catch {
            return []
        }
    })
const contentColorModules = new Set()
for (const catalog of catalogs) {
    if (!declaresColorSlot(catalog.contentContract?.slots)) continue
    contentColorModules.add(path.join(repoRoot, catalog.contentContract.module))
    for (const remix of catalogs) {
        if (remix.remixOf === catalog.key && typeof remix.contentSeed === "string") {
            contentColorModules.add(path.join(repoRoot, remix.contentSeed))
        }
    }
}
...
for (const viewDir of registeredViewDirs) {
    for (const filePath of walk(viewDir)) {
        if (!/\.(ts|tsx)$/.test(filePath)) continue
        const contentColors = contentColorModules.has(filePath)
        const lines = readFileSync(filePath, "utf8").split("\n")
        lines.forEach((line, index) => {
            if (contentColors && CONTENT_COLOR_LINE.test(line)) return
            if (HEX_COLOR.test(line) && !line.includes("&#")) {
                checkLine(
                    filePath,
                    index + 1,
