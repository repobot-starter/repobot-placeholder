...
// The 2026-09-15 publish outage: every CLI script gating on
// `path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)` was a
// SILENT no-op when invoked by absolute path through a symlink — node
// canonicalizes the main module's URL, argv[1] stays as typed. macOS mktemp
// workdirs live under /var/folders (symlink to /private/var/folders), so
// publish-templates.sh composed every template without its agent map and the
// publish gate refused all of them. The scripts now share
// scripts/lib/is-entry.mjs, which realpaths the comparison; this pins the
// exact invocation shape that regressed.
test("CLI scripts run their main when invoked through a symlinked path", () => {
    const tmp = mkdtempSync(path.join(os.tmpdir(), "is-entry-"))
    try {
        const link = path.join(tmp, "kernel")
        execFileSync("ln", ["-s", repoRoot, link])
        // No pack argument: main() prints usage and exits 1. A broken guard
        // skips main() entirely and exits 0 with no output — the silent
        // no-op this test exists to catch.
        const result = (() => {
            try {
                execFileSync("node", [path.join(link, "scripts", "generate-agent-map.mjs")], {
                    stdio: "pipe",
                })
                return { status: 0 }
            } catch (error) {
        ...
        assert.equal(
            result.status,
            1,
            "generate-agent-map.mjs invoked via a symlinked absolute path must run its main (usage exit 1), not silently no-op",
        )
    } finally {
...
test("compose expands a derived template over its base pack", () => {
    ...
    try {
        ...
        // The register is the base's unless the remix pins its own (the
        // services skins hold sitework while the base wears tideline), and a
        // remix's pinned page skeletons land verbatim.
        assert.equal(
            landing.style.preset,
            (remix.landing?.style ?? base.landing.style).preset,
            "the remix wears its resolved register",
        )
        if (remix.landing?.pages !== undefined) {
            assert.deepEqual(
                landing.pages,
                remix.landing.pages,
                "the remix's pinned pages must land verbatim",
            )
        }
        assert.equal(
...
test("compose declares a pack's platform services as manage domains", () => {
    // Platform services (proofing today) are Manage domains backed by
    // platform rows and the /__proofing runtime doors, not a content seed
    // in repobot.content.json — so packs declare them in the catalog's
    // platformServices array and compose merges them into the deploy
    // manifest's contentDomains (the one vocabulary the platform's Manage
    // rail gates on). Photography is the live declarer — and since it also
    // seeds the appointments content domain (the /book page), it pins the
    // merge of BOTH sources: seed domains first, then platform services,
    // no duplicates.
    const outputRoot = mkdtempSync(path.join(os.tmpdir(), "compose-pack-services-test-"))
    try {
        const key = "photography"
        const outputDir = path.join(outputRoot, key)
        execFileSync("bash", [path.join(repoRoot, "scripts", "compose-pack.sh"), key, outputDir], {
            stdio: "pipe",
            env: { ...process.env, REPOBOT_COMPOSE_SKIP_CODEGEN: "1" },
        ...
        assert.deepEqual(
            manifest.contentDomains,
            ["appointments", "proofing"],
            "the deploy manifest must declare the catalog's platform services as content domains",
        )
    } finally {
        rmSync(outputRoot, { recursive: true, force: true })
    }
