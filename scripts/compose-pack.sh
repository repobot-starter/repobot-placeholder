...
# declares capabilities. Packs can also declare `platformServices`
# (proofing today) — Manage domains backed by platform rows and runtime
# doors rather than a content seed; compose merges them into the same
# contentDomains vocabulary so the platform gates one way. formKinds
# mirrors the catalog's forms map (formKey
...
node -e '
    ...
    const contentSeedDomains = Object.keys(catalog.content ?? {}).filter(
        (key) => key !== "$comment",
    )
    const platformServices = (Array.isArray(catalog.platformServices)
        ? catalog.platformServices
        : []
    ).filter((value) => typeof value === "string")
    const contentDomains = [
        ...contentSeedDomains,
        ...platformServices.filter((service) => !contentSeedDomains.includes(service)),
    ]
