...
export function useAiChat(): AiChatThreadState {
    ...
    const submit = useCallback((message: string) => {
        ...
        const request = { id: requestId, message: trimmed, previousResponseId: previousResponseIdRef.current }
        ...
        // Demo previews stream a scripted reply — no AI endpoint exists there.
        if (import.meta.env.VITE_DEMO_MODE === "true") {
            void streamDemoAiChatResponse(request, abortController.signal, callbacks)
            return
        }
