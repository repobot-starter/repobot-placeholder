...
import { flowPaths } from "./flowShared"
...
/**
 * The member's books connection (manifest destination `/books`, packs/flow):
 * connect QuickBooks or Xero (instant in the sandbox), see which P&L
 * categories the grids can link against, disconnect any time.
 */
export default function BooksPage(): React.ReactElement {
    ...
    const connectBooks = async (provider: AccountingProvider): Promise<void> => {
        ...
        try {
            const result = await connect({
                ...
                refetchQueries: ["MyBooksConnection", "FlowLinkableCategories"],
            })
    ...
    const disconnectBooks = async (): Promise<void> => {
        try {
            await disconnect({ refetchQueries: ["MyBooksConnection", "FlowLinkableCategories"] })
            ...
            <header className={shared.header}>
                ...
                <p className={shared.subtitle}>
                    {connection
                        ...
                        : "Connect your accounting software so grid rows can link to live P&L categories."}
                </p>
            </header>
            ...
            {!connection ? (
                ...
                <div className={shared.card}>
                    <div className={shared.cardHeader}>
                        <h2 className={shared.cardTitle}>Linkable categories</h2>
                        <Badge tone="success">
                            {providerLabels[connection.provider] ?? connection.provider}
                        </Badge>
                    </div>
                    <p className={shared.mutedText}>
                        Grid rows linked to these categories auto-populate actuals every month.
                    </p>
                    {categories !== undefined ? (
                        <div className={shared.row}>
                            {[...categories.incomeCategories, ...categories.expenseCategories].map(
                                (category) => (
                    ...
                    <div className={shared.row}>
                        <Button onClick={() => navigate(flowPaths.templates)}>Open templates</Button>
                        <Button
                            variant="secondary"
                            size="sm"
