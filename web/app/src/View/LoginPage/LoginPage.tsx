...
const AUTH_LAYOUT_BY_PRESET: Record<string, "split" | "centered"> = {
    ...
    retroware: "centered",
    // The coastal register's door is a site-walk sign-in sheet: one card.
    tideline: "centered",
    // The flyer's door is one sticker card on the confetti ground.
    memphis: "centered",
    // The sign-painted counter: one board card.
    jacaranda: "centered",
    // The tabloid signs in at the newsroom door: one ruled press card.
    tabloid: "centered",
    // The campaign signs in at the will-call window: one card.
    gameday: "centered",
    // The drive-in signs in at the box office: one lobby card.
    creature: "centered",
    // The print shop signs in on one trimmed print: one card on the paper.
    riso: "centered",
    // The color deck signs in beside a flat field: the split panel.
    paintchip: "split",
    // The electrician's door is one hairline card on the drafting film.
    schematic: "centered",
    // The desert motel's front desk: the welcome panel in the sun.
    sunbelt: "split",
    // The cover's inside flap: the sign-in beside the cover photograph.
    crown: "split",
    // The counter's one card: a single framed panel on the lacquer.
    vanity: "centered",
    // The catalog studio's door is the order card: one ruled card.
    midcentury: "centered",
    // The photo-booth zine signs in on one taped snapshot card.
    photobooth: "centered",
    // The supper club's coat check: the welcome panel under the mirror ball.
    disco: "split",
    // The station's ticket window: the green sign panel beside the form.
    wayfinding: "split",
    // The listening room's door: one card on the warm sleeve.
    groove: "centered",
    // The gallery's front desk: one label card on the bare wall.
    inkblot: "centered",
    // The candy counter: the glossy welcome panel beside the form.
    bubblegum: "split",
    // The fridge door signs in on one taped print: one card on the paper.
    snapshot: "centered",
    // The ranch gate signs in at the chuck-wagon window: one ticket card.
    rodeo: "centered",
    // The sitter's wall signs in on one plain card on the stone.
    vitrine: "centered",
    // The headshot studio: one card on the white wall.
    seamless: "centered",
    // The liner notes: one card on the sleeve stock.
    liner: "centered",
    // The wedding registers sign in on one reply card on their own stock.
    seafog: "centered",
    harvest: "centered",
    carton: "centered",
    // The plant room signs in at the service ledger: one hairline card.
    plantroom: "centered",
    // The dispatch desk: the sign-in beside the storm panel.
    stormline: "split",
    // The garden studio's gate: one ruled card on the plaster.
    plaster: "centered",
    // The garden walk's gate: one quiet card in the fog.
    basalt: "centered",
    // The house's front door: one white card.
    whiteglove: "centered",
    // The club's front desk: the welcome panel beside the form.
    palmbeach: "split",
    // The parlor's front hall: one engraved card on the blush wall.
    parlor: "centered",
    // The trailhead: the sign-in card on the page like a note in a journal.
    trailhead: "centered",
    // The lounge: the sign-in card on warm cream, the textile edge above.
    hearth: "centered",
    // The press: one ruled card on the paper, like a title page.
    colophon: "centered",
    // Winter indoors: one white card on winter white.
    boreal: "centered",
    // The picnic: one card on the blanket.
    buttercream: "centered",
    // The dinner: one card on the navy.
    vineyard: "centered",
    // The birth house: one card on the plaster.
    adobe: "centered",
    // The clinic: one card on white.
    fogline: "centered",
    // The plaster studio: one card on the bone ground.
    limewash: "centered",
    // The creator's diary: one card on the limestone ground.
    miradouro: "centered",
    // The film creator: the photograph beside the card.
    sprocket: "split",
    galley: "split",
    // The waterfront agent: one card on the fog-pale ground.
    baylight: "centered",
    // The architect's monograph: one card on the gallery white.
    schist: "centered",
    // The galley proof: one card on the bone paper.
    galleyproof: "centered",
}
