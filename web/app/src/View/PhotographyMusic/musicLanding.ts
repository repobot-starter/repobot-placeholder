...
/** The record wall (`home.layout: "records"`): the first hero slide as one
 * full-bleed frame, the covers as a tight grid of square sleeves, then one
 * closing line and a text link to /book — no headline, no cards. */
function recordsHomeLanding(basePath: string): LandingConfig {
    return {
        style: { preset: PACK_REGISTERS["photography-music"] },
        shell: shell(basePath, ""),
        sections: [
            {
                ...
                variant: "sequence",
                content: {
                    items: heroSlides.slice(0, 1).map((image) => ({ media: imageMedia(image) })),
                    fullBleed: true,
            ...
            },
            {
                ...
                variant: "covers",
                content: {
                    items: home.records.covers.map((cover) => ({
                        media: imageMedia(cover.image),
            ...
            },
            {
                ...
                variant: "colophon",
                content: {
                    title: home.records.closing.line,
                    cta: { label: home.records.closing.cta, href: `${basePath}/book` },
                    align: "start",
                },
            },
        ],
...
/**
 * The client-galleries page: the proofing explainer, speaking to bands,
 * managers, and labels (what happens after the show) — which is also how
 * a photographer evaluating the template sees the proofing service
 * working as site content. The sample card opens the demo Late Hours room
 * on /proof, so a preview visitor can walk the real gate-and-choose flow;
    ...
    return {
        ...
        sections: [
            ...
            },
            {
                ...
                variant: "full-bleed",
                content: {
                    title: "No frames of your show yet?",
                    cta: { label: "Get me side-stage", href: `${basePath}/book` },
                },
            },
        ],
