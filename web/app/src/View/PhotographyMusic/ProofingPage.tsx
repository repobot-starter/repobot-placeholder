...
const accessKey = (slug: string): string => `photography-music-proof-access:${slug}`
const selectionKey = (slug: string): string => `photography-music-proof-selection:${slug}`
const sentKey = (slug: string): string => `photography-music-proof-sent:${slug}`
...
export default function ProofingPage(): React.ReactElement {
    ...
    return (
        <MarketingPage preset={PACK_REGISTERS["photography-music"]}>
...
function SelectionGallery({ room }: { room: RoomView }): React.ReactElement {
    ...
    const send = (): void => {
        ...
        void submitProofingSelection({
            ...
            picks: selectedIds,
            fallbackStorageKey: `photography-music-proof-fallback:${room.slug}`,
        }).then((result) => {
            setIsSending(false)
            if (result.status === "ok") {
