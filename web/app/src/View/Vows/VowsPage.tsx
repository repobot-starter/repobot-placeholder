...
export default function VowsPage({ page = "home" }: { page?: VowsPageKind }): React.ReactElement {
    ...
    if (page === "party") {
        return (
            <>
                <PageMeta
                    title={`${zine ? "Registry & the crew" : "Wedding party"} — ${couple.names}`}
                    siteName={couple.names}
                    description={`${zine ? "The registry, and the people" : "The people"} standing up with ${couple.partnerA} and ${couple.partnerB}.`}
                />
                <LandingRenderer config={resolved} />
            </>
        )
    ...
    return (
        <>
            {/* The classic subheadline already carries the date and the place. */}
            <PageMeta
                title={zine ? `${couple.names} — ${couple.weddingDateLabel}` : couple.names}
                siteName={couple.names}
                description={
                    zine
                        ? `${home.subheadline} ${couple.weddingDateLabel}, ${couple.venueShort}.`
                        : home.subheadline
                }
            />
            <LandingRenderer config={resolved} />
