 ...
 * counts, three numbered principles. A seed that sets `home.line` gets the
 * one-line home instead (`FundIndexLine`): the headline alone on the first
 * screen, the portfolio as a run of names, and the deck ask as one closing
 * line — and, wearing a register of its own, none of the achromatic pin.
 *
...
/**
 * The one-line home's own content (`home.line`): the slug over the line
 * (the fund's facts, in one row) and the label over the run of names.
 */
export interface FundIndexLine {
    badge: string
    portfolioLabel: string
}

function homeLine(): FundIndexLine | undefined {
    return "line" in home ? (home.line as FundIndexLine | undefined) : undefined
}

/** The page style: the achromatic pin is mono-utility's, so a one-line seed (its own register) drops it. */
function pageStyle(): LandingConfig["style"] {
    const preset = PACK_REGISTERS["fund-index"]
    return homeLine() !== undefined ? { preset } : { preset, overrides: FUND_INDEX_STYLE_OVERRIDES }
}
...
/** The one-line home: the line, the names, the ask. */
function lineHome(basePath: string, line: FundIndexLine): LandingConfig {
    return {
        style: pageStyle(),
...
export function homeLanding(basePath: string): LandingConfig {
    const line = homeLine()
    if (line !== undefined) return lineHome(basePath, line)
    return {
        style: pageStyle(),
        shell: shell(basePath, ""),
        sections: [
    ...
    return {
        ...
        sections: [
            ...
            },
            {
                id: "inquiry-form",
                type: "lead-form",
                // The deck ask as a real form — inquiries deliver through
                // the platform's managed forms pipeline (estate's contact
                // discipline). The direct channels trail the form: the
                // email stays deliberately plain text (no mailto:),
                // selectable for whatever mail client the founder actually
                // uses.
                variant: "detail-form",
                content: {
                    ...
                    channels: [
                        {
                            // A seed may name its own channel (`contact.emailLabel`); the index fund takes decks.
                            label: "emailLabel" in contact ? String(contact.emailLabel) : "Decks",
                            value: firm.email,
                        },
