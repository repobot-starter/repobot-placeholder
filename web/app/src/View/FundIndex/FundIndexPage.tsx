...
export default function FundIndexPage({ page = "home" }: { page?: FundIndexPageKind }): React.ReactElement {
    ...
    return (
        ...
        <div className={styles.blackout}>
            ...
            {/* On /contact, inquiries post formKey "fund-inquiry" through
                the managed forms pipeline: the partner gets an email and a
                dashboard entry with zero setup. The storage key scopes the
                sandbox fallback (and the "sent" state) to this pack. */}
            <LandingRenderer
                config={resolved}
                leadFormKey={page === "contact" ? "fund-inquiry" : undefined}
                leadStorageKey={page === "contact" ? "fund-index-contact" : undefined}
            />
        </div>
    )
}
