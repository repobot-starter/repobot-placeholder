...
export function useUsersViewModel(): UsersViewModel {
    ...
    const input = useMemo(() => buildUsersInput(search, tableSort), [search, tableSort])
    ...
    const loadMore = async (): Promise<void> => {
        ...
        try {
            await usersQuery.fetchMore({
                variables: { input: buildUsersInput(search, tableSort, pageInfo.endCursor) },
    ...
    const submit = async (formData: Record<string, unknown>): Promise<void> => {
        ...
        try {
            if (modal.mode === "create") {
                ...
                toast.publish({ title: "User created", tone: "success" })
            } else {
                await updateUser({
                    variables: {
                        input: {
                ...
                toast.publish({ title: "User saved", tone: "success" })
            }
            setModal(null)
        } catch (caught) {
...
function buildUsersInput(search: string, tableSort: DataTableSort, after?: string): UserConnectionInput {
    ...
    return {
        ...
        connection: {
            ...
            sort: [
                {
                    fieldName: SORT_FIELD_BY_COLUMN[tableSort.columnId] ?? "displayName",
