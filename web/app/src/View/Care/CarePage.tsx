...
import { booking, codeAppointments, codePractice, home, landingCopy, practice } from "./content"
...
export default function CarePage({ page = "home" }: { page?: CarePageKind }): React.ReactElement {
    ...
    if (page === "providers") {
        return (
            <>
                <PageMeta
                    title={`${landingCopy.nav.providers} — ${practice.name}`}
                    siteName={practice.name}
                    description={landingCopy.providersPage.subheadline}
                />
    ...
    if (page === "services") {
        return (
            <>
                <PageMeta
                    title={`${landingCopy.nav.services} — ${practice.name}`}
                    siteName={practice.name}
                    description={landingCopy.servicesPage.subheadline}
                />
                <LandingRenderer config={resolved} />
            </>
        )
    ...
    if (page === "new-patients") {
        return (
            <>
                <PageMeta
                    title={`${landingCopy.nav.newPatients} — ${practice.name}`}
                    siteName={practice.name}
                    description={landingCopy.newPatientsPage.subheadline}
                />
                <LandingRenderer config={resolved} />
            </>
        )
    ...
    if (page === "book") {
        return (
            <>
                <PageMeta
                    title={`${landingCopy.bookCtaLabel} — ${practice.name}`}
                    siteName={practice.name}
