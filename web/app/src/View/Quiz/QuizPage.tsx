...
// Glyph inks mirror the trivia-parlor palette in QuizPage.styles.css.ts
// (routed through the brand overlay the same way).
const quizGlyphInk = {
    accent: "var(--pack-accent, #d95d43)",
    soft: "#ffffff",
    line: "#eccfc7",
}
...
function loadBest(): BestScores {
    return readStoredJson<BestScores>(STORAGE_KEY, {})
}

function saveBest(best: BestScores): void {
    writeStoredJson(STORAGE_KEY, best)
}
                                ...
                                return (
                                    ...
                                    >
                                        <span className={styles.quizEmoji} aria-hidden>
                                            {/* The quiz's emoji seeds a generative mark
                                                inked from the pack palette — raw emoji
                                                read as template filler. */}
                                            <MarketingGlyph
                                                seed={`${quiz.title}${quiz.emoji}`}
                                                size={32}
                                                ink={quizGlyphInk}
                                            />
                                        </span>
