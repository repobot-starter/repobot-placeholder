 ...
 * pack's seed into it (the catalog's `content` overlay) as an INITIALIZER.
 * After compose the platform's managed domain owns this data — module and
 * domain agree at compose, then the domain leads (the live-data contract,
 * docs on the platform side: managed-services-live-data.md). Deployed
 * sites read the live document at runtime from their own origin's content
 * door (`/__content/doc.json`, hydrated below and refreshed on a poll +
 * tab focus), with the baked seed as the offline/SSR/first-paint
 * fallback; workspace previews receive management edits as preview-writes
 * of this file (Vite HMR hot-applies them). On a non-active surface (a
 * pack's preview route under another active pack) the code content
 * stands.
 *
...
const RUNTIME_CONTENT_DOCUMENT_PATH = "/__content/doc.json"
const RUNTIME_CONTENT_DOCUMENT_QUERY_PARAM = "repobotContentDocUrl"

/**
 * How often a page whose origin answers the content door re-reads it. A
 * management-UI edit lands in the platform's document store instantly; the
 * poll (plus the immediate tab-focus refetch below) is what carries it
 * into already-open tabs without a commit or redeploy. The response is a
 * few KB of business facts, served no-store — one small read per open tab
 * per interval.
 */
export const RUNTIME_CONTENT_REFRESH_INTERVAL_MS = 45_000
export const RUNTIME_CONTENT_REFRESH_INTERVAL_FAST_MS = 5_000

function notifyContentDocumentListeners(): void {
    documentVersion += 1
    for (const listener of [...documentListeners]) listener()
}

function jsonValueFingerprint(value: unknown): string | undefined {
    try {
...
function contentDocumentsEqual(a: unknown, b: unknown): boolean {
    if (Object.is(a, b)) return true
    const left = jsonValueFingerprint(a)
    const right = jsonValueFingerprint(b)
    return left !== undefined && right !== undefined && left === right
}

function setContentDocument(next: unknown): void {
    if (contentDocumentsEqual(currentDocument, next)) return
    currentDocument = next
    notifyContentDocumentListeners()
}

function debugContentHydration(message: string): void {
    console.debug(`[content] ${message}`)
}

function isHydrationDocument(value: unknown): value is Record<string, unknown> {
    return typeof value === "object" && value !== null && !Array.isArray(value)
}

/* Whether this page's origin has EVER served the content door. Sticky on
 * purpose: it separates the two hosting worlds once, on the first answer.
 * A deployed site (the router's door) keeps polling through transient
 * failures; a surface with no door at all — the sandbox dev server, a
 * workspace preview, a kernel checkout — fails its first fetch and stays
 * quiet forever (those surfaces get their freshness from HMR preview
 * writes instead). */
let runtimeDoorAnswered = false
let runtimeDoorEtag: string | undefined
let refreshTimer: ReturnType<typeof setTimeout> | undefined
let hydrationInFlight = false

function runtimeContentDocumentPath(): string {
    if (typeof window === "undefined") {
        return RUNTIME_CONTENT_DOCUMENT_PATH
    }
    try {
        const candidate = new URL(window.location.href).searchParams.get(RUNTIME_CONTENT_DOCUMENT_QUERY_PARAM)
        // Same-origin relative paths only: an absolute (or scheme-relative)
        // override would let any crafted link to a PUBLISHED site hydrate
        // attacker-controlled content into the page. Staged platform
        // surfaces always pass a same-origin path, so nothing legitimate
        ...
        // Malformed location falls back to the default same-origin door.
    }
    return RUNTIME_CONTENT_DOCUMENT_PATH
}

function scheduleRuntimeContentRefresh(): void {
    if (!runtimeDoorAnswered) return
    if (refreshTimer !== undefined) clearTimeout(refreshTimer)
    const refreshIntervalMs =
        runtimeDoorEtag !== undefined
            ? RUNTIME_CONTENT_REFRESH_INTERVAL_FAST_MS
            : RUNTIME_CONTENT_REFRESH_INTERVAL_MS
    refreshTimer = setTimeout(() => {
        refreshTimer = undefined
        // A hidden tab skips the fetch and just re-arms — the
        // visibilitychange listener below refetches the moment it is
        // looked at again, so the skip costs nothing.
        if (typeof document !== "undefined" && document.visibilityState === "hidden") {
            scheduleRuntimeContentRefresh()
            return
        }
        void hydrateRuntimeContentDocument()
    }, refreshIntervalMs)
}
...
async function hydrateRuntimeContentDocument(): Promise<void> {
    if (typeof window === "undefined") return
    const fetchRuntimeDocument = window.fetch?.bind(window)
    if (fetchRuntimeDocument === undefined) return
    if (hydrationInFlight) return
    hydrationInFlight = true
    const contentDocPath = runtimeContentDocumentPath()
    try {
        const response = await fetchRuntimeDocument(contentDocPath, {
            cache: "no-store",
            ...(runtimeDoorEtag !== undefined
                ? {
                      headers: {
                          "If-None-Match": runtimeDoorEtag,
                      },
                  }
                : {}),
        })
        ...
        if (response.status === 304) {
            runtimeDoorAnswered = true
            runtimeDoorEtag = etag ?? runtimeDoorEtag
            return
        }
        if (!response.ok) {
            debugContentHydration(
                `runtime content document unavailable at ${contentDocPath} (status ${response.status}); keeping baked document.`,
            )
            return
        }
        const hydratedDocument: unknown = await response.json()
        if (!isHydrationDocument(hydratedDocument)) {
            debugContentHydration(
                `runtime content document at ${contentDocPath} is not a JSON object; keeping baked document.`,
            )
            return
        }
        runtimeDoorAnswered = true
        runtimeDoorEtag = etag
        setContentDocument(hydratedDocument)
    } catch {
        debugContentHydration(
            `runtime content document fetch failed for ${contentDocPath}; keeping baked document.`,
        )
    } finally {
...
if (import.meta.hot) {
    ...
    import.meta.hot.accept("../../../../../repobot.content.json", (newModule) => {
        if (newModule !== undefined) {
            setContentDocument(newModule.default)
...
// The instant half of freshness for already-open tabs: coming back to the
// tab refetches immediately (a gym owner renames a class, switches to their
// site's tab, sees the rename), with the poll above covering tabs that
// never blur. Deliberately unconditional: it is also the recovery path for
// a deployed page whose FIRST fetch lost to a cold router (which never
// armed the poll) — one small fetch per focus-gain is negligible even on
// doorless surfaces, where it 404s quietly.
