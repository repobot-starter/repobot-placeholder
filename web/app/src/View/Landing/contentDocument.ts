...
async function hydrateRuntimeContentDocument(): Promise<void> {
    ...
    try {
        const response = await fetchRuntimeDocument(RUNTIME_CONTENT_DOCUMENT_PATH, { cache: "no-store" })
        if (!response.ok) {
            debugContentHydration(
                `runtime content document unavailable at ${RUNTIME_CONTENT_DOCUMENT_PATH} (status ${response.status}); keeping baked document.`,
            )
            return
        }
        ...
        if (!isHydrationDocument(hydratedDocument)) {
            debugContentHydration(
                `runtime content document at ${RUNTIME_CONTENT_DOCUMENT_PATH} is not a JSON object; keeping baked document.`,
            )
            return
        }
        ...
        debugContentHydration(
            `runtime content document fetch failed for ${RUNTIME_CONTENT_DOCUMENT_PATH}; keeping baked document.`,
        )
    }
}
