...
let durableDocument: unknown = currentDocument
...
type VisualDocApplySource = "durable" | "optimistic"

function applyLiveLandingDocument(document: unknown, source: VisualDocApplySource): void {
    ...
    if (source === "durable" && !landingDocumentGated(currentDocument)) {
        // The seq (recorded from the dev server's will-apply announcement,
        // which precedes this module update on the websocket) tells the
        // platform WHICH write this ack repainted — rapid-fire remix presses
        // coalesce, and an unkeyed ack used to clear the wrong overlay arm.
...
/** Applies a durably-landed landing document update (the normal HMR path). */
export function applyDurableLandingDocument(document: unknown): void {
    applyLiveLandingDocument(document, "durable")
}

/** Applies a preview-only optimistic landing document update (never acks). */
export function applyOptimisticLandingDocument(document: unknown): void {
    applyLiveLandingDocument(document, "optimistic")
}

/** Reverts optimistic landing edits back to the last durable HMR state. */
export function discardOptimisticLandingDocument(): void {
    applyLiveLandingDocument(durableDocument, "optimistic")
}
...
if (import.meta.hot) {
    ...
    import.meta.hot.accept("../../../../../repobot.landing.json", (newModule) => {
        if (newModule !== undefined) {
            applyDurableLandingDocument(newModule.default)
        }
    })
}
...
/** The currently applied landing document (durable or optimistic). */
export function getLandingDocument(): unknown {
 ...
 * bounded so a hostile document can't walk anywhere surprising. Six
 * segments reach a field of an item in a list nested inside a list item
 * (`groups.0.items.2.price`, `snapshots.1.frames.0`). */
const TEXT_FIELD_PATH = /^[a-zA-Z][a-zA-Z0-9]*(\.(?:[a-zA-Z][a-zA-Z0-9]*|\d{1,3})){0,5}$/
...
function withDocumentMedia(section: LandingSection, entry: Record<string, unknown>): LandingSection {
    ...
    for (const [list, value] of Object.entries(media)) {
        ...
        const next =
            list === "media" || list === "slides" || list === "items"
                ? applyListMedia(content, list, parsed)
                : applyPathMedia(content, list, parsed)
        if (next === undefined) {
            warnInvalid(
                `sections["${section.id ?? section.type}"].media["${list}"]`,
                list,
...
function isMediaRecord(value: unknown): boolean {
    return isRecord(value) && ("kind" in value || "src" in value)
}

/** A list the editor may paint: photographs, or items carrying one. */
function isPhotographList(value: unknown): value is unknown[] {
    return (
        Array.isArray(value) &&
        value.length > 0 &&
        value.every(isRecord) &&
        value.some((item) => isMediaRecord(item) || isMediaRecord(item.media))
    )
}

function valueAt(container: unknown, segments: string[]): unknown {
...
/** Immutable set of an EXISTING value at `segments` (never creates a path). */
function replaceValueAt(container: unknown, segments: string[], value: unknown): unknown {
    const [head, ...rest] = segments
    if (Array.isArray(container)) {
        const index = Number(head)
        if (!Number.isInteger(index) || index < 0 || index >= container.length) return undefined
        const replaced = rest.length === 0 ? value : replaceValueAt(container[index], rest, value)
        if (replaced === undefined) return undefined
        const next = [...container]
        next[index] = replaced
        return next
    }
    if (!isRecord(container) || !Object.hasOwn(container, head)) return undefined
    const replaced = rest.length === 0 ? value : replaceValueAt(container[head], rest, value)
    return replaced === undefined ? undefined : { ...container, [head]: replaced }
}

/**
 * Photograph overrides on any other content path: a list the code already
 * authors (`prints`, `snapshots`, `steps`, a nested `snapshots.0.frames`)
 * or a single media slot (`report.before`, `edition.bylineMedia`, written
 * as slot 0). Unlike the canonical lists, these never grow new structure —
 * a replace keeps the code list's length and a slot must already exist —
 * so an item's other fields (a sticker, a caption, a strip's frames) stay
 * bound to the item they were written for. Undefined when the path names
 * no photograph.
 */
function applyPathMedia(
    content: Record<string, unknown>,
    list: string,
    parsed: ParsedMediaOverride,
): Record<string, unknown> | undefined {
    if (!TEXT_FIELD_PATH.test(list)) return undefined
    const segments = list.split(".")
    const target = valueAt(content, segments)
    if (isMediaRecord(target)) {
        const src = parsed.kind === "replace" ? parsed.paths[0] : parsed.slots[0]
        if (src === undefined) return undefined
        return replaceValueAt(content, segments, overlayMediaOn(target, src)) as
            Record<string, unknown> | undefined
    }
    if (!isPhotographList(target)) return undefined
    const items = [...target]
    const slots: [number, string][] =
        parsed.kind === "replace"
            ? parsed.paths.map((src, index) => [index, src])
            : Object.entries(parsed.slots).map(([index, src]) => [Number(index), src])
    for (const [index, src] of slots) {
        if (index < items.length) items[index] = overlayMediaOn(items[index], src)
    }
    return replaceValueAt(content, segments, items) as Record<string, unknown> | undefined
}
