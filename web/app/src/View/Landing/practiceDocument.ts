 ...
 *   with: providers (name, credentials, role, tags, bio, photo ref), services,
...
export interface PracticeProvider {
    ...
    role?: string
    /** What the provider sees clients for, one line with " · " between tags — a directory's filter chips. */
    tags?: string
    bio: string
...
function parseProvider(entry: unknown, seen: Set<string>): PracticeProvider | undefined {
    ...
    const { providerId, name, credentials, role, tags, bio, photo } = entry
    ...
    if (tags !== undefined && !isText(tags, 200, 0)) return undefined
