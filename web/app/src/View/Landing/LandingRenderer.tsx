...
export function LandingRenderer({
    ...
    const join = (email: string, details?: Record<string, string>): void => {
        ...
        // A phone-first intake (the hero's `tel` contact) submits no email:
        // the number rides in details, and the marker holds the number.
        ...
        localStorage.setItem(leadStorageKey, email !== "" ? email : (details?.phone ?? ""))
        setJoined(true)
    }
...
export function LandingSectionView({
    ...
    joined: boolean
    onJoin: (email: string, details?: Record<string, string>) => void
}): React.ReactElement {
