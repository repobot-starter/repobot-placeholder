 ...
 * ships with a bundled ORIGINAL demo loop, named by its `loop` key — the
 * pages merge the generated waveform (View/Music/demoLoopTrack.ts) so this
 * file stays pure data the workspace Content panel can read and edit.
 * Paste a Spotify / Bandcamp / SoundCloud link into a track's `embedUrl`
 * and its player swaps to a click-to-load embed of the real recording
...
import type { ContentTrack } from "../Music/demoLoopTrack"
...
export interface BandTrack extends ContentTrack {
    cover?: BandImage
}
...
// Typed by annotation (not `satisfies` on the photos array): the workspace
...
export const pressKit: {
    ...
    photos: PressPhoto[]
    logos: { label: string; downloadHref: string; downloadLabel: string; onDark: boolean }[]
    stage: { lineup: string[]; specs: { label: string; value: string }[] }
