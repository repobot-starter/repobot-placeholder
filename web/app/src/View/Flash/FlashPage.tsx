...
// Glyph inks mirror the study-lamp palette in FlashPage.styles.css.ts
// (routed through the brand overlay the same way).
const deckGlyphInk = {
    accent: "var(--pack-accent, #5b4fc7)",
    soft: "#ffffff",
    line: "#c9c3ee",
}
...
function loadProgress(): SavedProgress {
    return readStoredJson<SavedProgress>(STORAGE_KEY, {})
}
                                ...
                                return (
                                    ...
                                    >
                                        <span className={styles.deckEmoji} aria-hidden>
                                            {/* The deck's emoji seeds a generative mark
                                                inked from the pack palette — raw emoji
                                                read as template filler. */}
                                            <MarketingGlyph
                                                seed={`${deck.title}${deck.emoji}`}
                                                size={32}
                                                ink={deckGlyphInk}
                                            />
                                        </span>
