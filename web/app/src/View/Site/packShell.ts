...
import { familyShell } from "../PhotographyFamily/familyShell"
...
import { influencerShell } from "../Influencer/influencerShell"
import { interiorsShell } from "../Interiors/interiorsShell"
...
import { editionShell } from "../WeddingEdition/editionShell"
...
const chromeByPack: Partial<Record<PackKey, (pagePath: string) => PackSiteChrome>> = {
    ...
    "photography-family": (pagePath) => ({
        shell: familyShell("", pagePath),
        preset: PACK_REGISTERS["photography-family"],
    }),
    wedding: (pagePath) => ({ shell: weddingShell("", pagePath), preset: PACK_REGISTERS.wedding }),
    "wedding-edition": (pagePath) => ({
        shell: editionShell("", pagePath),
        preset: PACK_REGISTERS["wedding-edition"],
    }),
    services: (pagePath) => ({
    ...
    interiors: (pagePath) => ({
        shell: interiorsShell("", pagePath),
        preset: PACK_REGISTERS.interiors,
    }),
    influencer: (pagePath) => ({
        shell: influencerShell("", pagePath),
        preset: PACK_REGISTERS.influencer,
    }),
