 ...
 * native waveform player, named by each mix's `loop` key — the pages merge
 * the generated waveform (View/Music/demoLoopTrack.ts) so this file stays
 * pure data the workspace Content panel can read and edit. Paste a
 * SoundCloud / Mixcloud / Spotify link into a mix's `embedUrl` and its
 * player swaps to a click-to-load embed of the real recording behind the
 * same frame.
 */
...
export interface Mix extends ContentTrack {
