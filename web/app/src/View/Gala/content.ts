 ...
 *
 * The evening opens on the masthead over the room, with the invitation's
 * engraved lines as their own card. Setting `event.name` (the guest of
 * honor, in script) turns the hero into the invitation card itself — the
 * particulars move onto its strip. The floor show, the dress-code
 * swatches, the hotel block, the decades wall, and the song requests
 * render only once they have entries; this evening leaves them empty.
 *
...
export const event = {
    ...
    nameNote: "",
    /** The page title: what the night is called. */
    title: "Vivienne turns sixty.",
    /** The hero line. */
    headline: "Vivienne turns sixty.",
    ...
    dateShort: "Thursday, December 31",
...
/** The floor show: what the night is built around, with photographs — none tonight. */
...
/** The dress code as fabric chips — black tie says it in the details instead. */
 ...
 * An object literal (not a bare helper call): `toastImage` is a top-level
 * media slot, and the content service edits a top-level slot by matching
 * its named export against a literal — a call expression here would make
 * the slot read-only in the Content panel. */
export const toastImage: SiteImage | null = {
    ...photo("toast", 1024, 1536, "A champagne coupe tower mid-pour by candlelight, chandelier bokeh behind"),
}
...
/** A photo wall through the years — none for this evening. */
...
/** Song requests from the reply cards — none for this evening. */
export const songs: {
    headline: string
    intro: string
...
export const landingCopy = {
    ...
    programKicker: "The program",
    /** The venue band's kicker. */
    venueKicker: "The room",
 ...
 * a one-page evening's masthead carries only the name and the one ask.
 */
...
/**
 * The home's composition. `program` is the evening above: the hero, the
 * running order, the fine print, the room. `stack` is the quiet party
 * page: the hero photograph with the headline and its date line
 * (`event.subtitle`) over it, a short band of lines under it
 * (`bandTitle` leads, `band` follows; a band with no title is one
 * centered line), the frames edge to edge at one size, the memory book
 * once it has entries, and the closing line over the RSVP link. The
 * program, details, and venue stay in this file for the `program` home;
 * the RSVP page is the same on both.
 */
export const home: {
    layout: HomeLayout
    bandTitle: string
