import type { LandingConfig, LandingSection } from "@ui"
...
import { experienceLabel, rangeWithDuration, sortRolesByDate, totalExperienceMonths, yearOf } from "./dates"
import { resumeShell } from "./resumeShell"

/**
 * A photograph on `person.portrait`, a role's or a project's `image`
 * (landscape reads best — the hero and the timeline crop 4:3). Seeds
 * declare their own shapes, so these are read structurally.
 */
...
function photoOf(entry: object, key: "portrait" | "image"): ResumePhoto | undefined {
    return key in entry ? ((entry as Record<string, unknown>)[key] as ResumePhoto | undefined) : undefined
}

const imageMedia = (photo: ResumePhoto) => ({
    kind: "image" as const,
    src: photo.src,
    alt: photo.alt,
    width: photo.width,
 ...
 * does all the art direction. The base seed is deliberately zero images.
 *
 ...
 *
 * A seed that photographs its person (see `ResumePhoto`) gets the
 * photographic résumé: `person.portrait` makes the hero the full-bleed
 * photograph, a role `image` hangs the experience on a photographic
 * timeline, a project `image` sets the work as newspaper stories. The
 * math is the same; print drops the photographs (ResumePage.print.css).
 * The base seed ships none, so its page stays type-only.
 */
export function resumeLanding(basePath: string, now: Date): LandingConfig {
    ...
    const portrait = photoOf(person, "portrait")
    const photographedRoles = roles.some((role) => photoOf(role, "image") !== undefined)
    const photographedProjects = projects.some((project) => photoOf(project, "image") !== undefined)
    return {
        ...
        sections: [
            {
                id: "hero",
                type: "hero",
                variant: portrait !== undefined ? "full-bleed-media" : "statement",
                content: {
                    ...
                    ...(portrait !== undefined ? { media: imageMedia(portrait) } : {}),
                },
            },
            {
                id: "summary",
            ...
            photographedRoles
                ? {
                      ...
                      content: {
                          ...
                          steps: ordered.map((role) => {
                              const image = photoOf(role, "image")
                              return {
                                  title: `${role.title} — ${role.company}`,
                                  description: `${rangeWithDuration(role, now)}. ${role.summary}`,
                                  label: `${yearOf(role.start)}`,
                                  ...(image !== undefined ? { media: imageMedia(image) } : {}),
                              }
                          }),
                      },
                  }
            ...
            },
            {
                id: "projects",
                type: "showcase",
                variant: photographedProjects ? "stories" : "card-grid",
                content: {
                    kicker: landingCopy.projects.kicker,
                    title: landingCopy.projects.title,
                    items: projects.map((project) => {
                        const image = photoOf(project, "image")
                        return {
                            title: project.title,
                            ...
                            eyebrow:
                                project.meta !== undefined
                                    ...
                                    : project.year,
                            // "" means "no destination yet" (the seed ships the
                            // field empty so the Content panel can fill it in) —
                            // normalize to undefined so the card stays unlinked.
