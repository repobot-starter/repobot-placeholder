...
export interface Project {
     ...
     * Leave "" until it's a real repo: the shipped demo destinations are
...
export interface ProfileLink {
    ...
    value: string
    /** Leave "" until the profile is real: the shipped placeholder
     * addresses point nowhere, and a dead external link is worse than
     * plain text. */
    url?: string
}
...
// Projects ship with `url: ""`: the demo repositories are fiction, so the
// cards render unlinked until you point them at your real repos. The field
// stays present (never omitted) so the workspace Content panel can fill it
// in — the content service edits existing literals, it cannot add
// properties.
...
// real profiles (set both `value` and `url`). The field stays present so
// the workspace Content panel can fill it in.
export const links: ProfileLink[] = [
    { label: "GitHub", value: "github.com/trivera", url: "" },
    { label: "Email", value: "tomas@rivera.example", url: "mailto:tomas@rivera.example" },
    { label: "Blog", value: "rivera.example/notes", url: "" },
    { label: "LinkedIn", value: "linkedin.com/in/tomasrivera", url: "" },
]
