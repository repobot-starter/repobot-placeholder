...
const SEAT_KIND_LABELS: Record<SeatKind, string> = { human: "Human", bot: "CPU", off: "Off" }
const DEFAULT_SEATS: SeatKind[] = ["human", "bot", "bot", "bot"]

// Muted takes on the four classic token colors, tuned for the dark board.
const SEAT_FILL: Record<SeatColor, { main: string; dark: string }> = {
    red: { main: "#d64537", dark: "#8c2018" },
    green: { main: "#3fa35c", dark: "#23663a" },
    yellow: { main: "#e3b341", dark: "#9c7a1c" },
    blue: { main: "#4a80d9", dark: "#2b4f91" },
}
...
function loadStats(): WinsTally {
    const parsed = readStoredJson<Partial<WinsTally>>(STATS_KEY, {})
    return {
        red: parsed.red ?? 0,
...
function seatLabel(seats: SeatKind[], seat: number): string {
    ...
    return `${SEAT_COLORS[seat]} (${kind === "bot" ? "CPU" : kind})`
}
...
/**
 * Home surface for the `ludo` pack: the classic four-color race. Seats are
 * configurable (human/CPU/off, 2-4 racers, full local hotseat), the rules
 * live in the pure `engine.ts`, and match wins persist in localStorage.
 * Fully client-side: no network, no assets — the board is inline SVG.
 */
    ...
    const recordWinIfOver = useCallback((prev: GameState, next: GameState): void => {
        if (!prev.over && next.over && next.placings.length > 0) {
            ...
            setStats((tally) => {
                ...
                writeStoredJson(STATS_KEY, updated)
                return updated
            })
        }
    }, [])
    ...
    const boardChrome = (
        <>
                ...
                return (
                    <g key={`yard-${color}`}>
                        <rect
                            ...
                            height={6 * CELL}
                            rx={10}
                            fill={SEAT_FILL[color].dark}
                            fillOpacity={0.32}
                            stroke={SEAT_FILL[color].main}
                            strokeWidth={2}
                        />
                        <rect
                            x={(origin.col + 1) * CELL}
            ...
            {RING_CELLS.map((cell, index) => {
                ...
                const fill = startSeat >= 0 ? SEAT_FILL[SEAT_COLORS[startSeat]].main : "#1c1c1f"
                return (
                    <rect
                        key={`ring-${index}`}
                        x={cell.col * CELL}
            ...
            <div className={styles.table}>
                <header className={styles.header}>
                    <div className={styles.wordmarkBlock}>
                        <span className={styles.wordmark}>Ludo</span>
                        <span className={styles.tagline}>
                            The classic four-color race. First to bring all four tokens home.
                        </span>
                    </div>
                </header>
                        ...
                        {game === null && (
                            <div className={styles.overlay}>
                                <div className={styles.overlayCard}>
                                    <div className={styles.overlayTitle}>Ludo</div>
                                    <div>
                            ...
                            <div className={styles.overlay}>
                                <div className={styles.overlayCard}>
                                    <div className={styles.overlayTitle}>
                                        {SEAT_COLORS[game.placings[0]]} wins
                                    </div>
                ...
                <footer className={styles.statusBar}>
                    <span>Local hotseat</span>
                    <span>
                        {game === null
                            ? "Setting up"
                            : game.over
                              ? "Match over"
                              : `${currentColor} to ${game.dice === null ? "roll" : "move"}`}
                    </span>
