...
/** Pause before the CPU acts, so its turn reads as an opponent thinking. */
...
function loadTally(): MatchTally {
    return { ...EMPTY_TALLY, ...readStoredJson<Partial<MatchTally>>(STATS_STORAGE_KEY, {}) }
}

/** Human log lines for one completed turn. */
function reportLines(report: TurnReport): string[] {
    const who = report.seat === "player" ? "You" : "CPU"
    const lines: string[] = []
    lines.push(
    ...
    if (report.botDecision === "koikoi") {
        lines.push("CPU calls koi-koi.")
    } else if (report.botDecision === "shobu") {
        lines.push("CPU calls shobu and banks the round.")
    }
    return lines
}

/** Home surface for the `hanafuda` pack: Koi-Koi against the CPU with the full 48-card flower deck. */
export default function HanafudaPage(): React.ReactElement {
    ...
    // The CPU acts on a delay whenever the engine hands it the turn.
    useEffect(() => {
                    ...
                    setToast({
                        ...
                        sub:
                            report.botDecision === "koikoi"
                                ? "CPU calls koi-koi — the round continues"
                                : "CPU banks the round.",
                    })
                }
            }
    ...
    useEffect(() => {
        ...
        setTally((existing) => {
            ...
            writeStoredJson(STATS_STORAGE_KEY, next)
            return next
        })
    }, [engine, engine.phase, version])
    ...
    const nextRound = (): void => {
        ...
        appendLog([`Round ${engine.round} — ${engine.dealer === "player" ? "you deal" : "the CPU deals"}.`])
        refresh()
    }
    ...
    const statusMessage =
        engine.phase === "selectHand"
            ...
            : engine.phase === "chooseFieldForHand"
              ? `${engine.pendingHandCard?.name} matches two — pick a field card`
              : engine.phase === "chooseFieldForDraw"
                ? `Flipped ${engine.pendingDrawnCard?.name} — pick a field card`
            ...
            <div className={styles.frame}>
                <header className={styles.header}>
                    <div className={styles.wordmarkBlock}>
                        <span className={styles.wordmark}>Hanafuda</span>
                        <span className={styles.tagline}>
                            Koi-koi with the 48-card flower deck — 花札・こいこい
                        </span>
                    </div>
                    <div className={styles.headerActions}>
                        <span className={styles.roundBadge}>
                            Round {engine.round} / {TOTAL_ROUNDS} —{" "}
                            {engine.dealer === "player" ? "you deal" : "CPU deals"}
                        </span>
                ...
                <div className={styles.layout}>
                    ...
                    <main className={styles.tableArea}>
                        <div className={styles.seatRow}>
                            <span className={styles.seatLabel}>CPU</span>
                            ...
                            <div className={styles.dialogScrim}>
                                <div className={styles.dialog}>
                                    ...
                                    <p className={styles.dialogBody}>
                                        Call <strong>koi-koi</strong> to press for a bigger hand (the CPU wins
                                    ...
                                    <h2 className={styles.dialogTitle}>
                                        {lastResult.winner === "player"
                                            ...
                                            : lastResult.winner === "bot"
                                              ? `CPU takes round ${lastResult.round}`
                        ...
                        {engine.phase === "matchOver" && (
                            <div className={styles.dialogScrim}>
                                <div className={styles.dialog}>
                                    ...
                                    <p className={styles.dialogBody}>
                                        Final score — You {playerTotal} · CPU {botTotal}
                                    </p>
                                    <div className={styles.dialogButtons}>
                        ...
                        <section className={styles.panel}>
                            ...
                            <div className={styles.logPanel}>
                                {log.map((line, index) => (
                                    <div
                                        key={index}
                                        className={line.startsWith("CPU") ? styles.logLineBot : undefined}
                                    >
                                        {line}
                                    </div>
                                ))}
                ...
                <footer className={styles.statusBar}>
                    <span>
                        ...
                        {engine.koiKoiCalled.bot ? "CPU koi-koi is live" : ""}
