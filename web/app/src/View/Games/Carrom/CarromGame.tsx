...
function playerName(player: PlayerIndex, mode: CarromMode): string {
    ...
    return mode === "bot" ? "CPU" : "Player two"
}
...
export default function CarromGame(props: CarromSettings & { resetToken: number }): React.ReactElement {
    ...
    useEffect(() => {
        ...
        settingsRef.current.onMessage?.("Player one breaks. Drag back from the striker to flick.")
...
const FIELD_LIGHT = "#202023"
const FIELD_DARK = "#19191c"
const FIELD_LINE = "rgba(255, 255, 255, 0.16)"
const FRAME = "#0c0c0d"
const POCKET_COLOR = "#060607"
const WHITE_COIN = "#f2f2f4"
...
/** Dark slate field, frame, rosettes, center circles and baselines. */
function drawBoard(ctx: CanvasRenderingContext2D): void {
    ...
    gradient.addColorStop(0, FIELD_LIGHT)
    gradient.addColorStop(1, FIELD_DARK)
    ...
    ctx.lineWidth = 1
    ctx.strokeRect(10.5, 10.5, BOARD_SIZE - 21, BOARD_SIZE - 21)
...
function drawCoin(ctx: CanvasRenderingContext2D, piece: CarromPiece): void {
    ...
    const palette =
        piece.kind === "queen"
            ? { fill: "#c2382c", rim: "#8c2018", dot: "#e6b0a8" }
            : piece.kind === "white"
              ? { fill: WHITE_COIN, rim: "#b9b9c0", dot: "#8f8f96" }
              : { fill: "#232326", rim: "#66666e", dot: "#4a4a52" }
...
function drawAimOverlay(ctx: CanvasRenderingContext2D, session: SessionState): void {
    ...
    // Power arc around the striker: solid white at full pull.
    ctx.strokeStyle = power > 0.85 ? "#fafafa" : "rgba(255, 255, 255, 0.55)"
    ctx.lineWidth = 5
    ctx.beginPath()
...
function drawScrim(ctx: CanvasRenderingContext2D, engine: CarromEngine, config: CarromSettings): void {
    ...
    ctx.fillText(title.split("").join(" "), BOARD_SIZE / 2, BOARD_SIZE / 2 - 10)
    ctx.font = "600 13px 'IBM Plex Mono', monospace"
    ctx.fillStyle = "rgba(255, 255, 255, 0.45)"
    ctx.fillText(
        engine.phase === "matchOver"
            ? `Final ${engine.matchScore[0]} — ${engine.matchScore[1]} · New match to play again`
