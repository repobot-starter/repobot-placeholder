...
function loadTally(): Tally {
    const saved = readStoredJson<Tally | null>(STATS_KEY, null)
    if (saved && typeof saved.white === "number" && typeof saved.black === "number") {
        return saved
    }
 ...
 * rules against a three-level CPU or a friend on the same table, played as a
...
export default function TawlaPage(): React.ReactElement {
    ...
    const seatName = useCallback(
        (player: Player): string =>
            mode === "1p" ? (player === "white" ? "You" : "CPU") : player === "white" ? "White" : "Black",
        [mode],
    )
    ...
    const finishTurn = useCallback(
        (final: Position, moverNow: Player): void => {
            ...
            if (nextScore[gameResult.winner] >= MATCH_TARGET) {
                ...
                setTally(nextTally)
                writeStoredJson(STATS_KEY, nextTally)
            } else {
                setPhase("gameOver")
            }
        },
    ...
    // CPU seat: roll automatically, then play the chosen turn checker by checker.
    useEffect(() => {
    ...
    const statusText =
        phase === "matchOver" && result
            ? `${seatName(result.winner)} wins the match ${score[result.winner]}–${score[opponent(result.winner)]}`
            : phase === "gameOver" && result
              ? `${seatName(result.winner)} — ${RESULT_LABEL[result.kind]}, +${result.points}`
              : phase === "rolling"
                ? "Rolling"
                : phase === "moving"
                  ? isBotSeat(mover)
            ...
            <div className={styles.console}>
                <header className={styles.header}>
                    <div className={styles.wordmarkBlock}>
                        <span className={styles.wordmark}>Tawla</span>
                        <span className={styles.tagline}>
                            Backgammon the café way. First to {MATCH_TARGET} points takes the match.
                        </span>
                    </div>
                    <div className={styles.headerActions}>
                        <button
                            ...
                            onClick={undoMove}
                            disabled={
                                !humanTurn || prefix.length === 0 || prefix.length === maxTurnLength(turns)
                            }
                        >
                            Undo
                        </button>
                        ...
                        <button className={styles.buttonPrimary} onClick={() => newGame(true)}>
                            New match
                        </button>
                    </div>
                </header>
                ...
                <div className={styles.layout}>
                    <aside className={styles.panelColumn}>
                        ...
                        <section className={styles.panel}>
                            <header className={styles.panelHeader}>Level</header>
                            <div className={styles.segmented}>
                                {LEVELS.map((option) => (
                                    <button
                                        key={option}
                        ...
                        <section className={styles.panel}>
                            <header className={styles.panelHeader}>Matches won</header>
                            <div className={styles.statRow}>
                                <span>{seatName("white")}</span>
                    ...
                    <main className={styles.boardColumn}>
                                ...
                                {(phase === "gameOver" || phase === "matchOver") && result && (
                                    <div className={styles.overlay}>
                                        ...
                                        <div className={styles.overlaySub}>
                                            {phase === "matchOver"
                                                ? `Final score ${score.white}–${score.black}`
                        ...
                        <div className={styles.controlsHint}>
                            Tap a checker, then a highlighted point — doubles play four moves
                        </div>
                    </main>
                        ...
                        <section className={styles.panel}>
                            ...
                            <div className={styles.muted}>
                                {whitePips === blackPips
                                    ? "Dead even race"
                                    : whitePips < blackPips
                                      ? `${seatName("white")} leads by ${blackPips - whitePips}`
                                      : `${seatName("black")} leads by ${whitePips - blackPips}`}
                            </div>
                        </section>
                ...
                <footer className={styles.statusBar}>
                    <span>{botThinking ? "Thinking" : "Ready"}</span>
                    <span>{statusText}</span>
                    <span>{mode === "1p" ? `CPU — ${LEVEL_LABELS[level]}` : "Two player"}</span>
                </footer>
            </div>
        </div>
    )
