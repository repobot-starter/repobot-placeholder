 ...
 * (bundled original audio, named by the excerpt's `loop` key — the page
 * merges the generated waveform via View/Music/demoLoopTrack.ts so this
 * file stays pure data the workspace Content panel can read and edit);
 * set `record.embedUrl` to a streaming link to swap it for a
 * click-to-load embed of the real song.
 */

import type { TrackImage } from "../Music/AudioPlayer"
import type { DemoLoopKey } from "../Music/demoLoopTrack"
...
export const record = {
    ...
    excerpt: {
        ...
        loop: "single-meridian" as const satisfies DemoLoopKey,
...
// An object literal (not a bare helper call): `portrait` is a top-level
...
export const portrait: SingleImage = {
    ...image("portrait", 1024, 1536, "Noor Vela, lit by a single window before dawn"),
}
