 ...
 * selection gallery. The photographer shares the link and the code after a
 * shoot; the client picks frames (persisted locally so they can return) and
 * sends the selection through the managed forms pipeline — the photographer
 * gets an email and a dashboard entry listing the exact frame ids.
 *
...
const accessKey = (slug: string): string => `photography-proof-access:${slug}`
const selectionKey = (slug: string): string => `photography-proof-selection:${slug}`
const sentKey = (slug: string): string => `photography-proof-sent:${slug}`
...
export default function ProofingPage(): React.ReactElement {
    ...
    return (
        <MarketingPage preset={PACK_REGISTERS.photography}>
...
function SelectionGallery({ album }: { album: ProofingAlbum }): React.ReactElement {
    ...
    const send = (): void => {
        ...
        void submitForm({
            ...
            fallbackStorageKey: `photography-proof-fallback:${album.slug}`,
        }).then(() => {
