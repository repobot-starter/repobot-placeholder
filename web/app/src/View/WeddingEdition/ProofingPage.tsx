...
const accessKey = (slug: string): string => `wedding-edition-proof-access:${slug}`
const selectionKey = (slug: string): string => `wedding-edition-proof-selection:${slug}`
const sentKey = (slug: string): string => `wedding-edition-proof-sent:${slug}`
...
export default function ProofingPage(): React.ReactElement {
    ...
    return (
        <MarketingPage preset={PACK_REGISTERS["wedding-edition"]}>
...
function SelectionGallery({ room }: { room: RoomView }): React.ReactElement {
    ...
    const send = (): void => {
        ...
        void submitProofingSelection({
            ...
            picks: selectedIds,
            fallbackStorageKey: `wedding-edition-proof-fallback:${room.slug}`,
        }).then((result) => {
            setIsSending(false)
            if (result.status === "ok") {
