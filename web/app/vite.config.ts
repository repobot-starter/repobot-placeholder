...
const appEntryPath = path.resolve(dirname, "src", "App.tsx")
const nonScopableTemplateRoots = new Set(["src/View/LoginPage", "src/View/Site"])
...
interface BuildTemplateScope {
    enabled: boolean
    reason: string
    activePackKey?: string
    activeTemplateRoot?: string
    templateRoots: ReadonlySet<string>
}
...
function normalizeTemplateRoot(homeViewDir: string): string | undefined {
    const normalized = homeViewDir.replace(/\\/g, "/").replace(/^\.?\/*/, "")
    const appRelative = normalized.startsWith("web/app/") ? normalized.slice("web/app/".length) : normalized
    return appRelative.startsWith("src/View/") ? appRelative.replace(/\/+$/, "") : undefined
}

function resolveTemplateRoot(
    key: string,
    catalogs: ReadonlyMap<string, PackCatalog>,
    stack: ReadonlySet<string> = new Set(),
): string | undefined {
    const catalog = catalogs.get(key)
    if (!catalog || stack.has(key)) {
        return undefined
    }
    if (typeof catalog.homeViewDir === "string") {
        return normalizeTemplateRoot(catalog.homeViewDir)
    }
    if (typeof catalog.remixOf === "string") {
        return resolveTemplateRoot(catalog.remixOf, catalogs, new Set([...stack, key]))
    }
    return undefined
}

function readBuildTemplateScope(): BuildTemplateScope {
    const packsDir = path.resolve(repoRoot, "packs")
    const activePath = path.resolve(packsDir, "active.json")
    let activePackKey: string | undefined
    try {
        const active = readJsonFile(activePath) as { key?: unknown }
        activePackKey = typeof active.key === "string" ? active.key : undefined
    } catch {
        return { enabled: false, reason: "active-pack-unreadable", templateRoots: new Set() }
    }
    if (!activePackKey) {
        return { enabled: false, reason: "active-pack-missing", templateRoots: new Set() }
    }

    const catalogs = new Map<string, PackCatalog>()
    for (const entry of readdirSync(packsDir, { withFileTypes: true })) {
        if (!entry.isDirectory()) {
            continue
        }
        const catalogPath = path.resolve(packsDir, entry.name, "catalog.json")
        try {
            const raw = readJsonFile(catalogPath) as PackCatalog
            const key = typeof raw.key === "string" ? raw.key : undefined
            if (key) {
                catalogs.set(key, raw)
            }
        } catch {
            // Ignore missing or invalid catalog files; fallback stays safe.
        }
    }

    const templateRoots = new Set<string>()
    for (const key of catalogs.keys()) {
        const root = resolveTemplateRoot(key, catalogs)
        if (root !== undefined) {
            templateRoots.add(root)
        }
    }

    const activeTemplateRoot = resolveTemplateRoot(activePackKey, catalogs)
    if (!activeTemplateRoot) {
        return {
            enabled: false,
            reason: "active-pack-unresolved",
            activePackKey,
            templateRoots,
        }
    }
...
function scopeTemplatePagesPlugin(scope: BuildTemplateScope): Plugin {
    const lazyImportPattern =
        /const\s+([A-Za-z0-9_]+)\s*=\s*lazy\(\(\)\s*=>\s*import\("(\.\/View\/[^"]+)"\)\)/g

    function ownerTemplateRoot(viewImportPath: string): string | undefined {
        const importPath = `src/${viewImportPath.slice("./".length).replace(/\\/g, "/")}`
        let ownerRoot: string | undefined
        for (const candidateRoot of scope.templateRoots) {
            if (importPath === candidateRoot || importPath.startsWith(`${candidateRoot}/`)) {
                if (!ownerRoot || candidateRoot.length > ownerRoot.length) {
    ...
    return {
        name: "scope-template-pages",
        transform(code, id) {
            if (!scope.enabled || path.resolve(id.split("?", 1)[0]) !== appEntryPath) {
                return null
            }
            let transformed = false
            const nextCode = code.replace(
                lazyImportPattern,
                (match, componentName: string, importPath: string) => {
                    const root = ownerTemplateRoot(importPath)
                    if (
                        root === undefined ||
                        root === scope.activeTemplateRoot ||
                        nonScopableTemplateRoots.has(root)
                    ) {
                        return match
                    }
                    transformed = true
                    return `const ${componentName} = ScopedTemplatePageStub`
                },
            )
            if (!transformed) {
                return null
            }
            const importLine = 'import ScopedTemplatePageStub from "./Config/ScopedTemplatePageStub"'
            if (nextCode.includes(importLine)) {
                return { code: nextCode, map: null }
            }
            const reactImportNeedle = 'import React, { lazy, Suspense } from "react"'
            if (nextCode.includes(reactImportNeedle)) {
                return {
                    code: nextCode.replace(reactImportNeedle, `${reactImportNeedle}\n${importLine}`),
                    map: null,
                }
            }
...
export default defineConfig(({ command }) => {
    const buildScope =
        command === "build" && process.env.REPOBOT_SCOPE_TEMPLATE_BUILD !== "0"
            ? readBuildTemplateScope()
            : { enabled: false, reason: "disabled", templateRoots: new Set<string>() }
    if (command === "build") {
        const detail = buildScope.enabled
            ? `${buildScope.activePackKey ?? "unknown"} (${buildScope.activeTemplateRoot ?? "unknown"})`
            : buildScope.reason
        console.log(`template scope: ${buildScope.enabled ? "enabled" : "fallback-full"} — ${detail}`)
    }
    ...
    return {
        plugins: [
            ...
            watchRootManifests(),
            templateStudio(),
            scopeTemplatePagesPlugin(buildScope),
        ],
        server: {
            port: 5173,
            proxy: apiProxy(),
            // Never paint Vite's red error overlay into the preview: the person
            // looking at this app is a site OWNER mid-remix, not a developer — a
            // full-screen stack trace over their site (the vanilla-extract "No
            // CSS for file" wedge, 2026-09-14) is pure terror with no action
            // they can take. Errors still log to console and terminal, which is
            // where agents and the platform's poisoned-server watchdog (which
            // restarts the stack) actually read them.
            hmr: { overlay: false },
