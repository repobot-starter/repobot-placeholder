...
import * as yara from "../../../src/View/Resume/yaraRemix.content"

type ContentModule = typeof base

const seeds: { name: string; module: ContentModule }[] = [
    { name: "dev", module: dev },
    { name: "yara", module: yara },
]
...
describe.each(seeds)("resume $name remix seed", ({ module }) => {
    ...
    it("only links projects and channels that have a real destination", () => {
        // Seeds ship most urls EMPTY ("") — demo destinations are fiction,
        // and an unlinked card beats navigation to a dead page. The field
        // stays present so the workspace Content panel can fill it in (the
        // content service edits existing literals, it cannot add
        // properties). A url that IS filled must be a real scheme, never a
        // placeholder domain.
...
describe("resume-yara remix seed", () => {
    it("rides the galley register", () => {
        expect(yaraCatalog.landing.style).toEqual({ preset: "galley" })
    })

    it("photographs the person, every kitchen and every project from its own folder", () => {
        // The photographs are what switch the builder to the photographic
        // résumé; a partial set would mix a pictured timeline with blank rows.
        const images = [
            yara.person.portrait,
            ...yara.roles.map((role) => role.image),
            ...yara.projects.map((p) => p.image),
        ]
        for (const image of images) {
            expect(image).toBeDefined()
            expect(image!.src).toMatch(/^\/resume-yara\//)
            expect(image!.alt).not.toBe("")
            for (const entry of image!.srcSet) expect(entry.src).toMatch(/^\/resume-yara\/.+-\d+w\.webp$/)
        }
    })
})
