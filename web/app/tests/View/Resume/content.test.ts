...
describe("resume content", () => {
    ...
    it("only links projects and channels that have a real destination", () => {
        // The shipped content leaves most urls EMPTY ("") — its destinations
        // are demo fiction, and an unlinked card beats navigation to a dead
        // page. The field stays present so the workspace Content panel can
        // fill it in (the content service edits existing literals, it cannot
        // add properties). A url that IS filled must be a real scheme, never
