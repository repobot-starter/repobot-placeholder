...
import { withDemoLoop } from "../../../src/View/Music/demoLoopTrack"
import {
    band,
    heroImages,
    pressKit,
...
describe("band content", () => {
    ...
    it("gives every track a playable bundled source with waveform peaks", () => {
        // Tracks carry a demo-loop KEY (content files stay pure data for the
        // workspace content service); withDemoLoop merges the generated
        // seconds/peaks at render level, so the playable shape is asserted
        // through the same merge the pages run.
        for (const record of records) {
            for (const track of record.tracks) {
                ...
                const playable = withDemoLoop(track)
                expect(playable.seconds, `${track.title} needs a duration`).toBeGreaterThan(15)
                expect(playable.peaks.length, `${track.title} needs peaks`).toBeGreaterThanOrEqual(48)
                for (const peak of playable.peaks) {
