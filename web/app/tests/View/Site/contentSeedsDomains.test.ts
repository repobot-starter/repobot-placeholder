...
describe("estate content seed (listings domain)", () => {
    // A tree composed for a remix of estate carries the remix's seed over
    // content.ts; its catalog's listings domain is the twin that applies.
    const estateContent = composedContentSeed("estate", estateCatalog.content as Record<string, unknown>)
    ...
    it("parses without dropping a single entry", () => {
        const raw = (estateContent as unknown as { listings: { entries: unknown[] } }).listings.entries
