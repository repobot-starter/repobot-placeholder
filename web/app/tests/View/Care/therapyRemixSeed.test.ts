...
import therapyCatalog from "../../../../../packs/care-therapy/catalog.json"
import {
...
/**
 * The care-therapy remix seed's parity gate (the services family's
...
const PUBLIC_PREFIX = "/care-therapy/"
...
function allImages(): { label: string; image: therapy.CareImage }[] {
    return [
        { label: "home.hero", image: therapy.home.hero },
        { label: "story.image", image: therapy.story.image },
        ...(therapy.spotlight !== null ? [{ label: "spotlight.image", image: therapy.spotlight.image }] : []),
        ...therapy.exhibits.items.map((item) => ({ label: `exhibit ${item.title}`, image: item.image })),
        ...Object.entries(therapy.providerPhotos).map(([providerId, image]) => ({
...
describe("care therapy remix seed", () => {
    it("mirrors the base module's export surface exactly", () => {
        ...
        expect(Object.keys(therapy).sort()).toEqual(Object.keys(base).sort())
    })

    it("keeps landingCopy a structural twin of the base's (careLanding reads every key)", () => {
        expect(keyShape(therapy.landingCopy)).toEqual(keyShape(base.landingCopy))
    })
    ...
    it("mirrors its own catalog's content seeds entry for entry, both domains", () => {
        ...
        expect(therapyCatalog.content.practice).toEqual(therapy.codePractice)
        expect(therapyCatalog.content.appointments).toEqual(therapy.codeAppointments)
        expect(parsePracticeContent({ practice: therapy.codePractice })).toEqual(therapy.codePractice)
        expect(parseAppointmentsContent({ appointments: therapy.codeAppointments })).toEqual(
            therapy.codeAppointments,
        )
    })
    ...
    it("meets the content contract's minimums", () => {
        ...
        expect(therapy.practice.name).not.toBe("")
        expect(therapy.practice.tagline).not.toBe("")
        expect(therapy.practice.email).toMatch(/@/)
        expect(therapy.codePractice.providers.length).toBeGreaterThanOrEqual(3)
        expect(therapy.codePractice.services.length).toBeGreaterThanOrEqual(6)
        expect(therapy.codePractice.insurance.length).toBeGreaterThanOrEqual(6)
        expect(therapy.codePractice.reviews.length).toBeGreaterThanOrEqual(3)
        expect(therapy.codePractice.newPatient.length).toBeGreaterThanOrEqual(3)
        expect(therapy.codeAppointments.types.length).toBeGreaterThanOrEqual(2)
        expect(therapy.codeAppointments.providers.length).toBeGreaterThanOrEqual(2)
        expect(therapy.clinicHours.length).toBeGreaterThanOrEqual(5)
        expect(Object.keys(therapy.providerPhotos).length).toBeGreaterThanOrEqual(3)
        expect(therapy.story.paragraphs.length).toBeGreaterThanOrEqual(2)
    })

    it("keeps ids on the contract grammar and joined across domains", () => {
        const practiceIds = new Set(therapy.codePractice.providers.map((provider) => provider.providerId))
        for (const provider of therapy.codePractice.providers) {
            expect(provider.providerId).toMatch(PRACTICE_ID_PATTERN)
        }
        for (const type of therapy.codeAppointments.types) {
            expect(type.typeId).toMatch(PRACTICE_ID_PATTERN)
        }
        for (const provider of therapy.codeAppointments.providers) {
            expect(practiceIds.has(provider.providerId), provider.providerId).toBe(true)
        }
        for (const providerId of Object.keys(therapy.providerPhotos)) {
            expect(practiceIds.has(providerId), providerId).toBe(true)
        }
        for (const providerId of practiceIds) {
            expect(therapy.providerPhotos[providerId], `portrait for ${providerId}`).toBeDefined()
        }
    })
    ...
    it("keeps session durations bookable and every window inside a day", () => {
        ...
        for (const entry of therapy.clinicHours) {
        ...
        const slots = generateAppointmentSlots(therapy.codeAppointments)
