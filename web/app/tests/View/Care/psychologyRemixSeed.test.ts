...
function allImages(): { label: string; image: psychology.CareImage }[] {
    return [
        ...
        ...(psychology.spotlight !== null
            ? [{ label: "spotlight.image", image: psychology.spotlight.image }]
            : []),
        ...psychology.exhibits.items.map((item) => ({ label: `exhibit ${item.title}`, image: item.image })),
