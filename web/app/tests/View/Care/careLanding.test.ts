...
import { codePractice, exhibits, home } from "../../../src/View/Care/content"
 ...
 *
 * The document is the one THIS tree composes: a care remix that pins its
 * own register and page skeletons (care-direct, care-therapy-duet,
 * care-psychology-studio, care-dental-bright) is reproduced by the
 * builders running on that remix's content seed; the original remixes
 * (therapy, psychology, dental) inherit the base's pages.
 */

const document = composedLandingSeed("care", careCatalog.landing)
...
describe("care catalog landing seed", () => {
    ...
    it("reproduces each page's code skeleton exactly", () => {
        ...
        expect(applySitePageDocument(providersLanding(""), "providers", document)).toEqual(
            inDocumentRegister(providersLanding("")),
        )
        expect(applySitePageDocument(servicesLanding(""), "services", document)).toEqual(
            inDocumentRegister(servicesLanding("")),
        )
        expect(applySitePageDocument(newPatientsLanding(""), "new-patients", document)).toEqual(
            inDocumentRegister(newPatientsLanding("")),
        )
        expect(applySitePageDocument(bookLanding(""), "book", document)).toEqual(
            inDocumentRegister(bookLanding("")),
        )
    })

    it("reorders a page when the documented skeleton changes", () => {
        // The last section this home builds (the clinic home's and the
    ...
    it("wears the luxe-light register and the full-width nav everywhere", () => {
        ...
        expect(careCatalog.landing.style).toEqual({ preset: "luxe-light" })
        for (const config of [
            homeLanding("", NOW),
            providersLanding(""),
            servicesLanding(""),
...
/**
 * The clinic home (a seed without `home.stack`): the live badge and both
 * asks on the hero, the team and the coverage strip on the page. The
 * `stack` home drops them to the inner pages (careStackHome.test.ts).
 */
describe.runIf(home.stack === undefined)("care clinic home", () => {
    ...
    it("keeps the booking ask on the hero, prefixed on the preview route", () => {
    ...
    it("renders business facts from the RESOLVED practice content", () => {
        ...
        // The home portraits band, or — when the home page is a directory
        // of the providers, which replaces the band — the directory's
        // cards; and the providers page always.
        const directory = exhibits.layout === "directory"
        const homeTeam = config.sections.filter((section) => section.id === "providers")
        const pageTeam = providersLanding("", edited).sections.filter((section) => section.id === "providers")
        expect(homeTeam).toHaveLength(directory ? 0 : 1)
        expect(pageTeam).toHaveLength(1)
        for (const team of [...homeTeam, ...pageTeam]) {
            expect((team.content as { members: { name: string }[] }).members.map((m) => m.name)).toEqual([
                "Dr. Test Provider",
            ])
        }
        if (directory) {
            const cards = config.sections.find((section) => section.id === "exhibits")
            const items = (
                cards?.content as { items: { title: string; eyebrow?: string; tags?: string[] }[] }
            ).items
            expect(items.map((item) => item.title)).toEqual(["Dr. Test Provider"])
            expect(items[0].eyebrow).toBe("MD")
            expect(items[0].tags).toEqual(["Grief", "Evenings"])
        }
