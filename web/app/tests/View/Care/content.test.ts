...
/**
 * True only where the root repobot.content.json cannot have been edited
 * since compose: the kernel checkout (no composed default exists) and the
 * template publish gate (REPOBOT_COMPOSE_GATE=1 on a freshly composed
 * stage). In a cloned project the document is the platform's write surface
 * — Manage edits land there — so the strict mirror assertion must not run.
 */
const documentIsPristine =
    !existsSync(path.join(REPO_ROOT, "packs/.defaults/repobot.content.json")) ||
    process.env.REPOBOT_COMPOSE_GATE === "1"
...
function allImages(): { label: string; image: CareImage }[] {
    return [
        ...
        ...(spotlight !== null ? [{ label: "spotlight.image", image: spotlight.image }] : []),
        ...exhibits.items.map((item) => ({ label: `exhibit ${item.title}`, image: item.image })),
...
describe("care content", () => {
    ...
    it.runIf(documentIsPristine)("mirrors the catalog's content seed entry for entry, both domains", () => {
        ...
        //
        // The seed is the RESOLVED one this tree ships: compose (and the
        // dev-pack switch) stamps the active catalog's — for a derived
        // template, the remix's — content domains into the root
        // repobot.content.json AND swaps the remix's contentSeed module
        // over content.ts, so the two must agree wherever a practice
        // domain has been stamped. Only a checkout no compose has touched
        // (the kernel default document carries no practice domain) falls
        // back to the base catalog, whose seed IS this module's content.
        const rootDocument = JSON.parse(
            readFileSync(path.join(REPO_ROOT, "repobot.content.json"), "utf8"),
        ) as { practice?: unknown; appointments?: unknown }
        const seed: { practice?: unknown; appointments?: unknown } =
            rootDocument.practice === undefined ? careCatalog.content : rootDocument
        expect(seed.practice).toEqual(codePractice)
