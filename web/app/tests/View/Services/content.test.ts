...
function allImages(): { label: string; image: SiteImage }[] {
    return [
        ...
        ...buildLog.steps.map((step) => ({ label: `build log ${step.label}`, image: step.image })),
        ...species.items.map((item) => ({ label: `species ${item.name}`, image: item.image })),
        ...(home.bannerImage !== null ? [{ label: "home.bannerImage", image: home.bannerImage }] : []),
    ]
}
...
describe("services content", () => {
    ...
    it("keeps the home teaser drawn from real projects", () => {
        // The teaser is optional (the title-card home shows restorations on
        // the projects page instead); whatever leads must be a real project.
        for (const featured of home.featuredProjects) {
            expect(projects).toContain(featured)
        }
    })

    it("stocks the title-card home: credit, caption, ticker, build log, materials", () => {
        // A storefront hero sells with the badge and the call button; the
        // title card sells with the photograph and what follows it, so a
        // title-card home without its sections would be a bare frame.
        if (home.hero.storefront) {
            expect(home.featuredProjects.length, "a storefront home leads with before/after").toBeGreaterThan(
                0,
            )
            return
        }
        // A caption-less cover or counter hero (the braids and makeup
        // restyles) is a different frame; its own seed suite stocks it.
        if (home.hero.caption === "") return
        expect(home.hero.credit).not.toBe("")
        expect(home.hero.caption).not.toBe("")
        expect(home.bannerImage).not.toBeNull()
        expect(nowBuilding.length).toBeGreaterThanOrEqual(2)
        for (const item of nowBuilding) expect(item.trim()).not.toBe("")
        expect(buildLog.title).not.toBe("")
        expect(buildLog.steps.length).toBeGreaterThanOrEqual(3)
        for (const step of buildLog.steps) {
            expect(step.title, `${step.label} title`).not.toBe("")
            expect(step.description, `${step.label} description`).not.toBe("")
            // The rail mark is small monospace: a month, a phase — not a sentence.
            expect(step.label.length, `${step.label} is a short mark`).toBeLessThanOrEqual(10)
        }
        expect(species.title).not.toBe("")
        expect(species.items.length).toBeGreaterThanOrEqual(3)
        for (const item of species.items) {
            expect(item.use, `${item.name} use`).not.toBe("")
            expect(item.description, `${item.name} description`).not.toBe("")
            // The specimen plate is a tall portrait.
            expect(item.image.height, `${item.name} plate is portrait`).toBeGreaterThan(item.image.width)
        }
    })
})

describe("services booking content", () => {
    it("meets the content contract's minimums", () => {
        // packs/services/catalog.json contentContract — the module
    ...
    it("mirrors the catalog's content seed entry for entry", () => {
        ...
        const seed = composedContentSeed("services", servicesCatalog.content)
