...
/**
 * The services-makeup-counter remix seed's parity gate — the same discipline as
 * the landscape/painting seeds in remixSeeds.test.ts and the hair seed in
 * hairRemixSeed.test.ts (those files are pinned, so this wave's seed gets
 * its own sibling suite). The seed is composed over the services pack's
...
const PUBLIC_PREFIX = "/services-makeup-counter/"

type ContentModule = typeof base

const module_: ContentModule = makeup
...
describe("services makeup remix seed", () => {
    ...
    it("mirrors its own catalog's appointments seed entry for entry", () => {
        expect(makeupCatalog.content.appointments).toEqual(module_.codeAppointments)
    ...
    it("fills the shade cards, the portfolio, the bridal process, the rate card and the travel rules", () => {
        expect(module_.palette.items.map((swatch) => swatch.name)).toEqual(["Bridal", "Editorial", "Lessons"])
        for (const swatch of module_.palette.items) expect(swatch.image, swatch.name).toBeDefined()
        const tags = new Set(module_.lookbook.items.flatMap((item) => item.tags))
        for (const tag of ["Editorial", "Bridal", "Red carpet"]) expect(tags, tag).toContain(tag)
        expect(module_.buildLog.steps.length).toBeGreaterThanOrEqual(3)
        expect(module_.priceMenu.groups.length).toBeGreaterThanOrEqual(2)
    ...
    it("keeps booking windows inside studio days — wedding dates stay contract-only", () => {
        // The seed's own promise: the widget books studio days (Wed–Fri);
        // weekend mornings belong to wedding contracts, never capacity-1
        // slots a stranger could take out from under a bride.
        for (const provider of module_.codeAppointments.providers) {
            for (const window of provider.windows) {
                expect([3, 4, 5]).toContain(window.day)
            }
        }
    })
})
