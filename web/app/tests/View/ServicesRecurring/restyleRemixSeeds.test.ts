/**
 * The restyled derived templates' parity gate for the recurring/booking
 * shape (services-recurring-turnover, services-lawncare-crew,
 * services-pest-creature, services-recurring-linen,
 * services-recurring-worth) — remixSeeds.test.ts's discipline, which is
 * pinned to the original trade seeds.
 *
...
import lawncareCatalog from "../../../../../packs/services-lawncare-crew/catalog.json"
import linenCatalog from "../../../../../packs/services-recurring-linen/catalog.json"
import pestCatalog from "../../../../../packs/services-pest-creature/catalog.json"
import turnoverCatalog from "../../../../../packs/services-recurring-turnover/catalog.json"
import worthCatalog from "../../../../../packs/services-recurring-worth/catalog.json"
...
const seeds: { name: string; module: ContentModule; publicPrefix: string; catalog: RemixCatalog }[] = [
    {
        name: "turnover",
        module: turnover,
        publicPrefix: "/services-recurring-turnover/",
        catalog: turnoverCatalog,
    },
    {
        name: "lawncare-crew",
        module: lawncare,
        publicPrefix: "/services-lawncare-crew/",
        catalog: lawncareCatalog,
    },
    { name: "pest-creature", module: pest, publicPrefix: "/services-pest-creature/", catalog: pestCatalog },
    { name: "linen", module: linen, publicPrefix: "/services-recurring-linen/", catalog: linenCatalog },
    { name: "worth", module: worth, publicPrefix: "/services-recurring-worth/", catalog: worthCatalog },
]
...
function allImages(module: ContentModule) {
    return [
        ...
        ...module.thread.messages.flatMap((message, index) =>
            message.photo !== undefined ? [{ label: `thread message ${index}`, image: message.photo }] : [],
        ),
    ]
}

describe.each(seeds)("recurring $name restyle seed", ({ module, publicPrefix, catalog }) => {
    it("is a derived template of services-recurring on its own key", () => {
        expect(catalog.key).toBe(publicPrefix.slice(1, -1))
        expect(catalog.remixOf).toBe("services-recurring")
    })
    ...
    it("meets the content contract's minimums", () => {
        ...
        // The home story is the base contract's either/or: the turnover
        // pair in full, or the classic trio in full.
        if (module.turnover.steps.length > 0 || module.roomReady.items.length > 0) {
            expect(module.turnover.steps.length).toBeGreaterThanOrEqual(3)
            expect(module.roomReady.items.length).toBeGreaterThanOrEqual(3)
            for (const step of module.turnover.steps) expect(step.image, step.title).toBeDefined()
        } else {
