...
describe("photography-family booking contract", () => {
    ...
    it("mirrors the catalog's content seed entry for entry", () => {
        ...
        // site already shows. The twin is the seed of the pack THIS tree
        // was composed for (a remix reseeds the domain and swaps this
        // module).
        const seed = composedContentSeed("photography-family", familyCatalog.content)
...
describe("photography-family proofing demo fixtures", () => {
    ...
    it("derives stable, unique frame ids inside each demo proofing album", () => {
        // The processed file's base name: the album's frames are named for it.
        const cover = albums[0].images[0]
        expect(cover.src.endsWith(`/${photoId(cover)}-${cover.width}w.webp`)).toBe(true)
        for (const album of demoProofingAlbums) {
