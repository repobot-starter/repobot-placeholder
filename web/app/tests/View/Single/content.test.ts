...
describe("single content", () => {
    ...
    it("bundles a playable excerpt with waveform peaks", () => {
        // The excerpt carries a demo-loop KEY (content files stay pure data
        // for the workspace content service); the playable shape is asserted
        // through the same merge the page runs.
        expect(publicAssetPresent(path.join(PUBLIC_DIR, record.excerpt.audioSrc))).toBe(true)
        const playable = withDemoLoop(record.excerpt)
        expect(playable.seconds).toBeGreaterThan(15)
        expect(playable.peaks.length).toBeGreaterThanOrEqual(48)
