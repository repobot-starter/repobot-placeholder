...
describe("photography proofing demo fixtures", () => {
    it("ships demo rooms with preview PINs, clients, and images", () => {
        ...
        // The funnel screenshots the sample room (the /galleries walkthrough
        // opens it — the base's Harlow room, a remix's own); it must look
        // populated.
        const sample = demoProofingAlbums.find((album) => album.slug === galleries.sample.slug)
        expect(sample, `${galleries.sample.slug} is the funnel's demo gallery`).toBeDefined()
    ...
    it("derives stable, unique frame ids inside each demo proofing album", () => {
        expect(photoId({ ...albums[0].images[0], src: "/photography/portraits-01-1536w.webp" })).toBe(
            "portraits-01",
        )
        for (const album of demoProofingAlbums) {
