...
describe("LinkPage", () => {
    ...
    it("renders the profile and every link from content.ts", () => {
        ...
        for (const social of socials) {
            // A social chip is a link only when its URL names a real profile
            // (a path beyond the platform homepage); the shipped bare-homepage
            // placeholders must render as inert badges, not dead navigation.
            const hasProfilePath = new URL(social.url).pathname.replace(/\/+$/, "") !== ""
            if (hasProfilePath) {
                expect(screen.getByRole("link", { name: social.label })).toBeTruthy()
            } else {
                expect(screen.queryByRole("link", { name: social.label })).toBeNull()
                expect(screen.getByLabelText(social.label)).toBeTruthy()
            }
        }
    })
