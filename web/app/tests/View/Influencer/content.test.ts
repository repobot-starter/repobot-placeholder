...
/** The catalog seed this tree's content module mirrors (a remix's, once composed). */
const seed = composedContentSeed("influencer", influencerCatalog.content)
...
describe("influencer content", () => {
    ...
    it("mirrors the catalog's links seed entry for entry, in the same order", () => {
        ...
        expect(seed.links.entries).toEqual(links)
    })
    ...
    it("mirrors the catalog's looks seed entry for entry (minus photographs)", () => {
        ...
        const facts = looks.map(({ image: _image, ...entry }) => entry)
        expect(seed.looks.entries).toEqual(facts)
    })
