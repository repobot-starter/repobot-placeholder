...
describe("wedding proofing demo fixtures", () => {
    it("ships demo rooms with preview PINs, clients, and images", () => {
        ...
        // The sample walkthrough opens the /galleries sample room (Nora &
        // June on this pack, the first couple on a remix); it must look
        // populated.
        const sample = demoProofingAlbums.find((album) => album.slug === galleries.sample.slug)
        expect(sample, `${galleries.sample.slug} is the sample demo gallery`).toBeDefined()
    ...
    it("derives stable, unique frame ids inside each demo proofing album", () => {
        ...
        // the same at every width the responsive verb emitted.
        const cover = albums[0].images[0]
        const baseName = (cover.srcSet[0]?.src.split("/").pop() ?? "").replace(/-\d+w\.webp$/, "")
        expect(baseName).not.toBe("")
        expect(photoId(cover)).toBe(baseName)
        for (const entry of cover.srcSet) {
            expect(photoId({ ...cover, src: entry.src })).toBe(baseName)
        }
        for (const album of demoProofingAlbums) {
