...
import kernelContentDocument from "../../../../../repobot.content.json"
...
const registeredPacks: RegisteredPack[] = [
    ...
    { key: "wedding-edition", catalog: weddingEditionCatalog, home: () => weddingEditionHome("") },
]
...
function composeLandingDocument(catalog: RegisteredPack["catalog"]): Record<string, unknown> {
    ...
    try {
        ...
        // Every real composed tree carries the content document too — a
        // pack with a business-content seed (photography's appointments
        // domain) merges into it during the overlay.
        writeFileSync(
            path.join(tree, "repobot.content.json"),
            JSON.stringify(kernelContentDocument, null, 4) + "\n",
        )
