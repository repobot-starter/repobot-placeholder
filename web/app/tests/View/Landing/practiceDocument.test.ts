...
describe("parsePracticeContent", () => {
    ...
    it("carries a provider's tags line through, and drops a provider whose tags aren't text", () => {
        const tagged = { ...validDocumentProvider, tags: "Grief · Evenings" }
        const parsed = parsePracticeContent({
            practice: {
                providers: [tagged, { ...validDocumentProvider, providerId: "dr-bad", tags: ["Grief"] }],
            },
        })
        expect(parsed.providers).toEqual([tagged])
    })
