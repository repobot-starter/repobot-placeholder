...
import architectureCatalog from "../../../../../packs/interiors-architecture/catalog.json"
import {
...
import * as architecture from "../../../src/View/Interiors/architectureRemix.content"

/**
 * The interiors-architecture remix seed's parity gate (the services
 * family's remixSeeds discipline, the therapy/hair seeds' sibling — and
 * the interiors family's first). The seed (packs/README.md "Derived
...
const PUBLIC_PREFIX = "/interiors-architecture/"

type ContentModule = typeof base

const module_: ContentModule = architecture
...
describe("interiors architecture remix seed", () => {
    ...
    it("mirrors its own catalog's content seeds entry for entry, both domains", () => {
        ...
        expect(architectureCatalog.content.projects.entries).toEqual(facts)
        expect(parseContentProjects({ projects: { entries: facts } })).toEqual(facts)
        expect(architectureCatalog.content.appointments).toEqual(module_.codeAppointments)
