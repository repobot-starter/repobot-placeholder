...
import cfoCatalog from "../../../../../packs/interiors-cfo/catalog.json"
import {
...
import * as cfo from "../../../src/View/Interiors/cfoRemix.content"

/**
 * The interiors-cfo remix seed's parity gate (the services family's
 * remixSeeds discipline, the architecture and wedding seeds' sibling). The seed
...
const PUBLIC_PREFIX = "/interiors-cfo/"

type ContentModule = typeof base

const module_: ContentModule = cfo
...
describe("interiors cfo remix seed", () => {
    ...
    it("mirrors its own catalog's content seeds entry for entry, both domains", () => {
        ...
        expect(cfoCatalog.content.projects.entries).toEqual(facts)
        expect(parseContentProjects({ projects: { entries: facts } })).toEqual(facts)
        expect(cfoCatalog.content.appointments).toEqual(module_.codeAppointments)
