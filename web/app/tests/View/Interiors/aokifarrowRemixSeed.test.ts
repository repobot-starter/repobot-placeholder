...
import aokifarrowCatalog from "../../../../../packs/interiors-architecture-aokifarrow/catalog.json"
import {
...
/**
 * The interiors-architecture-aokifarrow remix seed's parity gate (the
 * interiors-architecture seed's sibling). The seed (packs/README.md "Derived
 * templates") is composed over the interiors pack's content module
 * verbatim, so it must remain a structural twin of `content.ts`: the same
 * export surface, the contract's minimums met, its catalog's content
 * seeds mirrored entry for entry (projects minus the code-owned
 * photographs, appointments exactly), and every image real under the
 * seed's own public directory. These tests fail the moment the pack's
 * contract moves without its seed — the drift that would otherwise
 * surface as a broken published template. The monograph home is the
 * seed's `home.monograph`: its feature photographs are real too, and the
 * plates (the featured projects) and the hero share one shape, so the
 * stack keeps one size.
 */

const PUBLIC_DIR = path.resolve(__dirname, "../../../public")
const PUBLIC_PREFIX = "/interiors-architecture-aokifarrow/"

type ContentModule = typeof base

const module_: ContentModule = aokifarrow

const monograph = (aokifarrow.home as { monograph?: InteriorsMonograph }).monograph
...
function allImages(module: ContentModule) {
    return [
        ...
        ...(monograph === undefined
            ? []
            : [
                  { label: "feature spread", image: monograph.feature.spread },
                  ...monograph.feature.figures.map((figure, index) => ({
                      label: `feature figure ${index}`,
                      image: figure.image,
                  })),
                  ...(monograph.feature.plate !== undefined
                      ? [{ label: "feature plate", image: monograph.feature.plate.image }]
                      : []),
              ]),
    ]
}

describe("interiors aokifarrow remix seed", () => {
    ...
    it("mirrors its own catalog's content seeds entry for entry, both domains", () => {
        ...
        expect(aokifarrowCatalog.content.projects.entries).toEqual(facts)
        expect(parseContentProjects({ projects: { entries: facts } })).toEqual(facts)
        expect(aokifarrowCatalog.content.appointments).toEqual(module_.codeAppointments)
    ...
    it("sets the monograph home: a feature, an index heading, and a closing line", () => {
        expect(monograph).toBeDefined()
        expect(monograph?.heroCaption).not.toBe("")
        expect(monograph?.feature.headline).not.toBe("")
        expect(monograph?.feature.body.split(/\n\s*\n/).length).toBeGreaterThanOrEqual(2)
        expect(monograph?.feature.figures.length).toBeGreaterThanOrEqual(2)
        expect(monograph?.indexTitle).not.toBe("")
        expect(monograph?.closing).not.toBe("")
        expect(monograph?.closingCta).not.toBe("")
    })

    it("holds the hero and the plates to one shape, so the stack keeps one size", () => {
        const shapes = [
            module_.home.heroImage,
            ...module_.projects.filter((p) => p.featured === true).map((p) => p.image),
        ]
        expect(new Set(shapes.map((image) => (image.width / image.height).toFixed(4))).size).toBe(1)
    })
})
