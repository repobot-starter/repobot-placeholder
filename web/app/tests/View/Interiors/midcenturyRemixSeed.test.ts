...
import * as midcentury from "../../../src/View/Interiors/midcenturyRemix.content"

/**
 * The interiors-midcentury remix seed's parity gate (the services
 * family's remixSeeds discipline, the architecture seed's sibling). The
 * seed (packs/README.md "Derived templates") is composed over the
 * interiors pack's content module verbatim, so it must remain a
 ...
 * entry (projects minus the code-owned photographs, appointments
 * exactly), and every image real under the seed's own public directory —
 * the catalog home's case files and pieces included. These tests fail
 * the moment the pack's contract moves without its seed — the drift that
 * would otherwise surface as a broken published template.
 */

const PUBLIC_DIR = path.resolve(__dirname, "../../../public")
const PUBLIC_PREFIX = "/interiors-midcentury/"

type ContentModule = typeof base

const module_: ContentModule = midcentury
...
function allImages(module: ContentModule) {
    return [
        ...
        ...module.caseStudies.befores.map((entry) => ({ label: `before ${entry.slug}`, image: entry.image })),
        ...module.pieces.items.map((piece) => ({ label: `piece ${piece.number}`, image: piece.image })),
    ]
}

describe("interiors midcentury remix seed", () => {
    ...
    it("mirrors its own catalog's content seeds entry for entry, both domains", () => {
        ...
        expect(midcenturyCatalog.content.projects.entries).toEqual(facts)
        expect(parseContentProjects({ projects: { entries: facts } })).toEqual(facts)
        expect(midcenturyCatalog.content.appointments).toEqual(module_.codeAppointments)
