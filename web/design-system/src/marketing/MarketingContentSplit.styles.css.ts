...
import { ctaPrimary, emojiPanel, mediaImage, section, sectionKicker, LARIAT_ROOT, spot1 } from "./shared.css"
...
/** `report`: the case file is the evidence, so it takes the wider column. */
export const gridReport = style({
    gridTemplateColumns: "0.9fr 1.1fr",
    "@media": {
        "(max-width: 860px)": { gridTemplateColumns: "1fr" },
    },
...
/** `report`: the claim the card proves, set at display size. */
export const headlineReport = style({
    fontSize: `calc(clamp(34px, 5vw, 58px) * ${marketing.display.scale})`,
    lineHeight: 1.02,
    margin: "0 0 22px",
})
...
/* `lariat`: the bullets are stars, lettered in turquoise. */
globalStyle(`${LARIAT_ROOT} ${bulletMark}`, { fontSize: 0 })

globalStyle(`${LARIAT_ROOT} ${bulletMark}::before`, {
    content: '"★"',
    fontSize: 15,
    color: spot1,
})
