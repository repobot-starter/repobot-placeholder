...
/**
 * The intake card: a surface plate that asks the one question, offers the
 * answers as icon tiles, and takes a callback. It carries the page's own
 * surface and ink, so it reads the same over a photograph or the paper.
 */
...
export const intakeHeading = style({
...
export const intakeForm = style({
    display: "grid",
    gap: 10,
})

export const intakeChoices = style({
    display: "grid",
...
export const intakeLegend = style({
    position: "absolute",
    width: 1,
    height: 1,
    overflow: "hidden",
...
export const intakeChoice = style({
    position: "relative",
    display: "flex",
    ...
    selectors: {
        "&:hover": { color: marketing.color.text },
        "&:focus-within": { outline: `2px solid ${marketing.color.accent}`, outlineOffset: 2 },
    },
})

export const intakeChoiceOn = style({
    color: marketing.color.text,
    borderColor: marketing.color.accent,
    boxShadow: `inset 0 0 0 1px ${marketing.color.accent}`,
})

export const intakeRadio = style({
    position: "absolute",
    opacity: 0,
    pointerEvents: "none",
})

export const intakeInput = style([input, { width: "100%", boxSizing: "border-box" }])

export const intakeButton = style([button, { width: "100%", justifyContent: "center" }])
...
/*
 * `atomic` (midcentury): the order card — hung from an ink rule, labels in
 * tracked geometric caps, and the channel links and confirmation in the
 * burnt-orange ink (the mustard is for fields of color, not small type).
 */
globalStyle(`${ATOMIC_ROOT} ${detailForm}`, {
    borderTop: `2px solid ${marketing.color.text}`,
})

globalStyle(`${ATOMIC_ROOT} ${detailLabel}`, {
    fontSize: 11.5,
    fontWeight: 600,
...
globalStyle(`${ATOMIC_ROOT} ${channelLink}, ${ATOMIC_ROOT} ${confirmation}`, {
    color: spot1,
})

// mirrorball: pill controls, but the message box keeps soft corners — a
// fully round textarea reads as a stretched button.
globalStyle(`${MIRRORBALL_ROOT} ${detailTextarea}`, { borderRadius: 18 })
