 ...
 * `full-stop` accents only the headline's closing punctuation (the
 * catalog cover's mustard period) and sets the words bare — a headline
 * without closing punctuation renders bare too.
 * `last-line` accents the headline's whole final line when the author set
 * line breaks ("NOT A PORTRAIT.\nYOUR SATURDAY.") and falls back to
 * `last-word` for a one-line headline; the breaks render as line breaks.
 */
export type MarketingAccentPlacement = "last-word" | "first-word" | "none" | "full-stop" | "last-line"
 ...
 * keeps the original grammar — the last word pops. `joined` marks an
 * accent that attaches to the lead without a space (`full-stop`).
 */
export function splitAccentWord(
    headline: string,
    placement: MarketingAccentPlacement = "last-word",
): { lead: string; accentWord: string; trail: string; joined?: true } {
    const trimmed = headline.trim()
    if (placement === "full-stop") {
        const stop = /[.!?\u2026]+$/.exec(trimmed)
        if (stop === null || stop.index === 0) return { lead: trimmed, accentWord: "", trail: "" }
        return { lead: trimmed.slice(0, stop.index), accentWord: stop[0], trail: "", joined: true }
    }
    ...
    if (placement === "last-line" && trimmed.includes("\n")) {
        const cut = trimmed.lastIndexOf("\n")
        return { lead: trimmed.slice(0, cut).trim(), accentWord: trimmed.slice(cut + 1).trim(), trail: "" }
    }
    return {
