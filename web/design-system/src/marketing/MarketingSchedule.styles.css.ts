...
/*
 * `lariat` day rows: the bill's program. Each day in wood type, a stitched
 * rule between days, the times lettered in turquoise.
 */
globalStyle(`${LARIAT_ROOT} ${rows}`, {
    borderTop: `4px double ${marketing.color.line}`,
})

globalStyle(`${LARIAT_ROOT} ${row}`, {
    gridTemplateColumns: "230px 1fr",
    padding: `${scaledSpace(26)} 0`,
    borderBottom: `2px dashed ${marketing.color.line}`,
    "@media": {
        "(max-width: 640px)": { gridTemplateColumns: "1fr" },
    },
})

globalStyle(`${LARIAT_ROOT} ${row} ${dayLabel}`, {
    textWrap: "balance",
...
globalStyle(`${LARIAT_ROOT} ${rowSessions}`, { gap: 14 })

globalStyle(`${LARIAT_ROOT} ${rowSession}`, {
    display: "grid",
    gridTemplateColumns: "92px auto minmax(0, 1fr)",
    "@media": {
        "(max-width: 640px)": { gridTemplateColumns: "84px minmax(0, 1fr)" },
    },
})

globalStyle(`${LARIAT_ROOT} ${rowSession} > :nth-child(n + 4)`, { gridColumn: "2 / -1" })

globalStyle(`${LARIAT_ROOT} ${rowSession} ${detail}`, {
    "@media": {
        "(max-width: 640px)": { gridColumn: "2 / -1" },
    },
})

globalStyle(`${LARIAT_ROOT} ${rowSession} ${time}`, {
    fontFamily: marketing.font.display,
    fontSize: 15,
...
globalStyle(`${LARIAT_ROOT} ${rowSession} ${sessionTitle}`, { fontSize: 17, fontWeight: 700 })

globalStyle(`${LARIAT_ROOT} ${rowSession} ${detail}`, { fontSize: 14.5, fontStyle: "italic" })
