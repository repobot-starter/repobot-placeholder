...
export const LANDING_SECTION_VARIANTS = {
    ...
    "feature-grid": variantsOf<MarketingFeatureGridVariant>()([
        "cards-3up",
        "icon-list",
        "bento",
        "checklist",
    ]),
    steps: variantsOf<MarketingStepsVariant>()([
        "numbered-cards",
        "timeline",
        "horizontal-rail",
        "message-thread",
    ...
    pricing: variantsOf<MarketingPricingVariant>()([
        "tiers",
        "price-list",
        "tickets",
        "price-tags",
    ...
    "content-split": variantsOf<MarketingContentSplitVariant>()([
        "media-right",
        "media-left",
        "report",
        "feature",
    ...
    stats: variantsOf<MarketingStatsVariant>()(["row", "cards", "bars"]),
    ...
    "cta-banner": variantsOf<MarketingCtaBannerVariant>()([
        "card",
        "full-bleed",
        "ticket",
        "classified",
...
export const LANDING_MEDIA_DEPENDENT_VARIANTS: Partial<Record<LandingSectionType, readonly string[]>> = {
    hero: ["full-bleed-media", "masthead-overlay", "front-page", "invitation", "pinboard"],
}
 ...
 * second paired presentation ever ships. content-split's `report` joins on
 * the same grounds: its case file pairs a before and an after photograph
 * (plus the job's rows) that no other split layout renders, and `feature`
 * on the same: its figures and plate are photographs only it sets. hero's
 * `pinboard` joins too: its `prints` pile only it lays out, and a hero
 * rolled onto it without prints would pin up an empty board.
 */
export const LANDING_PAIRED_MEDIA_VARIANTS: Partial<Record<LandingSectionType, readonly string[]>> = {
    gallery: ["before-after"],
    "content-split": ["report", "feature"],
    hero: ["pinboard"],
}
