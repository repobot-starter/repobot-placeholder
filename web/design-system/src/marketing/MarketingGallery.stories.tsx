...
/**
 * A roll printed as a 35mm contact sheet: sprocket bands, frame numbers
 * from `firstFrame`, the stock along the edge, and the editor's grease
 * pencil — a ring on the keeper, a strike through the reject, notes in hand.
 */
export const ContactSheet: Story = {
    args: {
        variant: "contact-sheet",
        kicker: "Roll 12",
        title: "Saturday on the pier",
        edgeCode: "Kodak Portra 400",
        firstFrame: 23,
        lightbox: true,
        items: photoItems.map((item, index) => ({
            ...item,
            mark: index === 1 ? ("circle" as const) : index === 4 ? ("cross" as const) : undefined,
            note: index === 1 ? "THIS ONE" : index === 4 ? "blink" : undefined,
        })),
    },
    decorators: [
        (Story) => (
...
/**
 * The album as photo-booth strips (`photobooth` + `zine`): four frames a
 * strip at resting tilts, a strip's label tag and handwritten caption
 * riding its first frame's `note` and `caption`.
 */
export const PhotoStrip: Story = {
    args: {
        variant: "photo-strip",
        kicker: "Photo booth",
        title: "Straight out of the booth",
        lightbox: true,
        items: photoItems.map((item, index) => ({
            ...item,
            note: index === 0 ? "Take one" : index === 4 ? "Engaged!!" : undefined,
            caption:
                index === 0 ? "The first strip." : index === 4 ? "Paper crowns, party of two." : undefined,
        })),
    },
    decorators: [
        (Story) => (
