...
let lastWriteSource: "durable" | "optimistic" = "durable"
...
/** Whether the latest write came from durable HMR or optimistic preview input. */
export function themeContractOverrideSource(): "durable" | "optimistic" {
    return lastWriteSource
}

/** Replaces the live contract and notifies subscribers (dev HMR only). */
export function setThemeContractOverride(
    next: RepobotThemeConfig,
    source: "durable" | "optimistic" = "durable",
): void {
    override = next
