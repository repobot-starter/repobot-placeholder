 ...
 *
 * The store fires in dev (HMR contract events) AND in baked/production
 * bundles when the platform preview bridge applies an optimistic theme
 * document (remix in the wizard's baked preview). Where nothing ever
 * applies a contract — tests, untouched production pages — the store never
 * fires and everything below is inert: components see the exact build-time
 * constants.
 */
            ...
            (["light", "dark"] as const).map((mode) => {
                ...
                const declarations = (
                    [
                        ...
                        [marketing.color.inkOnAccent, overlay.inkOnAccent],
                        [marketing.color.inkOnSpot, overlay.inkOnSpot],
...
let durableThemeContract: RepobotThemeConfig = rawThemeContract

type VisualDocApplySource = "durable" | "optimistic"

function applyThemeContract(contract: unknown, source: VisualDocApplySource): void {
    lastContractGated = themeDocumentForeign(contract)
    const gated = gateThemeDocument(contract)
    if (source === "durable") {
        durableThemeContract = gated
    }
    setThemeContractOverride(gated, source)
}

/** Applies a durably-landed theme document update (the normal HMR path). */
export function applyDurableThemeDocument(contract: unknown): void {
    applyThemeContract(contract, "durable")
}

/** Applies a preview-only optimistic theme document update (never acks). */
export function applyOptimisticThemeDocument(contract: unknown): void {
    applyThemeContract(contract, "optimistic")
}

/** Reverts optimistic theme edits back to the last durable HMR state. */
export function discardOptimisticThemeDocument(): void {
    // Discard is preview-only state repair, so it must not look like a
    // durable repaint ack to the platform.
    setThemeContractOverride(durableThemeContract, "optimistic")
    lastContractGated = false
}
...
if (import.meta.hot) {
    ...
    import.meta.hot.on(THEME_CONTRACT_EVENT, (contract) => {
        applyDurableThemeDocument(contract)
    })
...
/* The store-driven CSS re-application runs EVERYWHERE, not just under HMR:
 * the platform's preview bridge (previewBridge.ts) posts optimistic theme
 * documents into BAKED bundles too (the wizard look step's remix), where
 * import.meta.hot is undefined. Gating this subscription on hot left baked
 * previews applying only the React-facing structural presets (nav, mode) —
 * every token in the injected stylesheet (palette, the --pack-accent
 * family, radii, fonts) silently no-oped, so remix visibly changed layout
 * while colors never moved (owner repro, 2026-09-11). The store never
 * fires unless someone applies a contract, so this stays inert in tests
 * and on untouched production pages. */
subscribeThemeContract(() => {
    const override = getThemeContractOverride()
    if (override !== null) {
        applyThemeContractVars(override)
        if (!lastContractGated && themeContractOverrideSource() === "durable") {
