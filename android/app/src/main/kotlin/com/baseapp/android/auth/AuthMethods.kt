...
enum class AuthMethod(val key: String) {
    ...
    /**
     * True for the redirect-flow OAuth providers; the key doubles as the
     * provider path on the auth API ("<provider>/start").
     */
    val isOAuthProvider: Boolean
        get() = when (this) {
            GOOGLE, APPLE, GITHUB, FACEBOOK, DISCORD, X, LINKEDIN -> true
            EMAIL_CODE, PASSWORD, ANONYMOUS -> false
        }
