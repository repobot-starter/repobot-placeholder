...
/**
 * All content for the blog, mirrored from the web pack's content.ts.
 * Post bodies use the markdown subset that BlogMarkdown parses. To make
 * this template yours, replace the author and posts — nothing else needs
 * to change. Posts render newest-first by date.
 */
object BlogContent {
    data class Author(
        val name: String,
    ...
    const val TITLE = "Field Notes"
    const val TAGLINE =
        "Short essays on building software carefully, published whenever one is ready."

    val author = Author(
        name = "Sam Okafor",
        role = "Product engineer",
        bio = "I build small, sturdy software and write down what I learn. " +
            "Previously at a mapping startup; now independent.",
        initials = "SO",
    )
    ...
    val posts = listOf(
        Post(
            ...
            date = "2026-06-18",
            tags = listOf("process", "teams"),
            summary = "Most design reviews fail because they try to do three jobs at once. " +
                "Split them, cap the clock, and the meeting fixes itself.",
            body = """
Every bad design review I've sat through was three meetings wearing one calendar slot: a status update, a brainstorm, and a decision. Nobody agreed which one we were in, so we did all three badly.
...
We've run this for about a year. The twenty-minute cap gets broken maybe one review in ten, and when it does, it's because the problem statement was fuzzy — which is exactly the thing worth finding out early.
        ...
        Post(
            ...
            summary = "You probably don't need the 40 kB library. A constrained input format " +
                "plus eighty lines of scanner is often the sturdier choice.",
            body = """
There's a reflex, when a feature needs to render formatted text, to reach for the biggest markdown library on the registry. Sometimes that's right. But when *you* control the input — config files, release notes, docs your own team writes — the calculus flips.
...
When you genuinely need CommonMark — user-generated content, imported documents — take the dependency and don't look back. But notice how rarely that's the situation you're actually in.
        ...
        Post(
            ...
            date = "2026-05-07",
            tags = listOf("craft", "tools"),
            summary = "One append-only text file per project, three prefixes, and a Friday " +
                "sweep. It has outlived every app I've tried.",
            body = """
I've tried every notes app with a keyboard shortcut and a dark mode. What survived is a text file.
...
I keep the file in the repo, next to the code it describes. It goes through review like everything else, which has a nice side effect: teammates correct my decision log while the context is still warm.
        ...
        Post(
            ...
            date = "2026-04-12",
            tags = listOf("craft", "process"),
            summary = "The weekend project graveyard is full of second Sundays. Scope to one " +
                "sitting, cut the login page, and publish something embarrassing.",
            body = """
The failure mode of the weekend project isn't running out of skill. It's running out of *weekend* — arriving at Sunday night with an auth flow, an empty README, and no feature.
...
The projects that grew into something real were, without exception, the ones that shipped small first. The graveyard is full of the other kind.
        ...
        Post(
            ...
            date = "2026-03-25",
            tags = listOf("writing"),
            summary = "That little '4 min read' label is a contract with the reader. " +
                "Breaking it costs more than leaving it off.",
            body = """
A reading-time label looks like a convenience feature. It's actually a promise: *this is how much of your attention I'm asking for.*
...
The deeper habit is treating *all* metadata as promises. The date says this is current. The tag says this is about what it claims. The summary says the middle won't surprise you. Small labels, honestly kept, are most of what trust in a publication is made of.
    ...
    fun allTags(list: List<Post> = posts): List<String> {
    ...
    fun sortedPosts(list: List<Post> = posts): List<Post> = list.sortedByDescending { it.date }
}
