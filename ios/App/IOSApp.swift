...
@main
struct IOSApp: App {
  /// APNs device-token callbacks only arrive through an app delegate; the
  /// adaptor forwards them to PushComponent (see PushComponent.swift).
  ...
  var body: some Scene {
    WindowGroup {
      RootView()
        ...
        .environmentObject(store.billingStore)
        .environmentObject(store.avatarStore)
