...
struct SitterGameView: View {
  ...
  @State private var bestPay = DeviceStorage.readNumber(key: SitterEngine.bestPayKey, fallback: 0)
    ...
    for event in events {
      if case .shiftEnded(let result) = event {
        ...
        if result.pay > bestPay {
          bestPay = result.pay
          DeviceStorage.writeNumber(key: SitterEngine.bestPayKey, value: result.pay)
        }
      }
    }
    renderTick += 1
