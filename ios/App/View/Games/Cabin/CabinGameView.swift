...
final class CabinSession {
  ...
  private(set) var bestRating = DeviceStorage.readNumber(key: CabinSession.bestRatingKey, fallback: 0)
  ...
  func tick(now: Date) {
    ...
    if engine.phase == .landed, engine.stars > bestRating {
      ...
      DeviceStorage.writeNumber(key: Self.bestRatingKey, value: bestRating)
    }
  }
}
