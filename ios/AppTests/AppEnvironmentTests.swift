...
final class AppFlavorTests: XCTestCase {
  ...
  func testParsesWebOriginAndTreatsEmptyAsAbsent() throws {
    let withOrigin = try ConfigLoader.parse(flavor: .dev, dictionary: [
      "GRAPHQL_URL": "https://example.com/prefix__graphql__request__api",
      "AUTH_MODE": "builtin",
      "AUTH_REDIRECT_URL": "baseapp-dev://auth/callback",
      "WEB_ORIGIN": "https://myapp.example",
    ])
    XCTAssertEqual(withOrigin.webOrigin?.absoluteString, "https://myapp.example")
    XCTAssertEqual(withOrigin.resolvedWebOrigin?.absoluteString, "https://myapp.example")

    // Empty (unstamped) WEB_ORIGIN means no billing surfaces, not a boot
    // failure — unlike GRAPHQL_URL it never fails closed.
    let withoutOrigin = try ConfigLoader.parse(flavor: .dev, dictionary: [
      "GRAPHQL_URL": "https://example.com/prefix__graphql__request__api",
      "AUTH_MODE": "builtin",
      "AUTH_REDIRECT_URL": "baseapp-dev://auth/callback",
      "WEB_ORIGIN": "",
    ])
    XCTAssertNil(withoutOrigin.webOrigin)
    XCTAssertNil(withoutOrigin.resolvedWebOrigin)
  }

  func testResolvedWebOriginAdoptsTheStampedLanHostInSandbox() {
    // The Stamp Sandbox Config build phase rewrites only GRAPHQL_URL's
    // localhost host to the Mac's LAN IP; the runtime derivation carries
    // that rewrite over to the localhost WEB_ORIGIN (keeping its own port,
    // where the dev web server listens).
    XCTAssertEqual(
      AppConfig.resolveWebOrigin(
        webOrigin: URL(string: "http://localhost:5173"),
        graphqlURL: URL(string: "http://192.168.1.20:5001/demo/us-central1/graphql__request__api")!
      )?.absoluteString,
      "http://192.168.1.20:5173"
    )

    // Simulators keep localhost on both URLs: nothing to rewrite.
    XCTAssertEqual(
      AppConfig.resolveWebOrigin(
        webOrigin: URL(string: "http://localhost:5173"),
        graphqlURL: URL(string: "http://127.0.0.1:5001/demo/us-central1/graphql__request__api")!
      )?.absoluteString,
      "http://localhost:5173"
    )

    // Deployed origins pass through untouched.
    XCTAssertEqual(
      AppConfig.resolveWebOrigin(
        webOrigin: URL(string: "https://myapp.example"),
        graphqlURL: URL(string: "https://us-central1-proj.cloudfunctions.net/graphql__request__api")!
      )?.absoluteString,
      "https://myapp.example"
    )

    XCTAssertNil(AppConfig.resolveWebOrigin(webOrigin: nil, graphqlURL: URL(string: "https://example.com")!))
  }

  func testAvatarImageProcessingTargetSizeDownscalesOnlyOversizedImages() {
    // Long edge over the cap scales down preserving aspect ratio.
    XCTAssertEqual(
      AvatarImageProcessing.targetSize(for: CGSize(width: 4000, height: 3000), maxDimension: 1024),
      CGSize(width: 1024, height: 768)
    )
    XCTAssertEqual(
      AvatarImageProcessing.targetSize(for: CGSize(width: 500, height: 2048), maxDimension: 1024),
      CGSize(width: 250, height: 1024)
    )
    // Images already within the cap pass through untouched.
    XCTAssertEqual(
      AvatarImageProcessing.targetSize(for: CGSize(width: 800, height: 600), maxDimension: 1024),
  ...
  func testThemeCatalogMatchesWebDarkAndLightAnchorValues() {
    ...
    // Anchors come from repobot.theme.json's brand block (the same source
    // web/design-system/src/theme/themeConfig.ts derives its tokens from).
    XCTAssertEqual(dark.colors.accent, Color(hex: "#90caf9"))
    XCTAssertEqual(light.colors.accent, Color(hex: "#1f6feb"))
