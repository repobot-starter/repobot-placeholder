...
final class AuthMethodsTests: XCTestCase {
  ...
  func testPreservesConfiguredOrder() {
    ...
    XCTAssertEqual(
      resolveAuthMethods(methodsValue: "apple, google, email-code"),
      [.apple, .google, .emailCode]
    )
  }
  ...
  func testIgnoresUnknownNamesAmongKnownOnes() {
    XCTAssertEqual(
      resolveAuthMethods(methodsValue: "sms, google, magic-beans, anonymous"),
      [.google, .anonymous]
    )
  }
  ...
  func testToleratesWhitespaceAndCase() {
    ...
    XCTAssertEqual(resolveAuthMethods(methodsValue: " Apple "), [.apple])
  }
  ...
  func testOAuthProviderClassificationMirrorsTheRegistry() {
    XCTAssertEqual(
      AuthMethod.allCases.filter(\.isOAuthProvider),
      [.google, .apple, .github, .facebook, .discord, .x, .linkedin]
    )
    for provider in AuthMethod.allCases where provider.isOAuthProvider {
      XCTAssertNotNil(provider.oauthDisplayName, "\(provider) needs a button label")
    }
  }
}
