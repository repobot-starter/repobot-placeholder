...
copy of the repobot-shopify-beauty example store**, and the user's existing theme
...
6. **Replace the demo copy.** This repo ships as a skincare example: the
   hero, routine builder, ingredient glossary, before-and-after rows, bundle
   callout, and product key-ingredients and how-to-use blocks carry demo
   content in section schema defaults and in `templates/index.json` /
   `templates/product.json`. Overwrite it with the user's real copy; drop
   example sections their store has no use for.
