...
**Repobot Glow**, a clean, clinical-but-warm skincare example store. It ships
with a complete storefront — a product-spotlight hero, skin-type-badged
product grid, an AM/PM routine builder, an ingredient glossary, honest
before-and-after rows with a results disclaimer, a full-routine bundle
callout, and key-ingredients and how-to-use blocks on the product page — all
as demo copy the user will replace with their own. The demo copy lives in
section `{% schema %}` defaults and in the JSON templates
(`templates/index.json`, `templates/product.json`); reusable UI strings
(labels, empty states) live in `locales/en.default.json`. When the user asks
for their own store, edit that copy — don't bolt new sections alongside the
demo ones. Keep the copy calm and evidence-flavored: no hype, and no medical
claims (the before-and-after disclaimer stays).
