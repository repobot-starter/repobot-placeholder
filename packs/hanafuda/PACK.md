...
Client-only vertical pack: Koi-Koi, the classic two-player Japanese hanafuda (flower card) game, played against the CPU at `web/app/src/View/Games/Hanafuda/`.

## What ships

- Koi-Koi table page (owns `/` when this pack is active; otherwise preview at `/hanafuda`) — monochrome modern table, capture trays grouped by card type, yaku toasts, koi-koi/shobu dialog, 6-round scoreboard
- The full 48-card hanafuda deck drawn as inline SVG (`cards.tsx`) — 12 months x 4 cards in flat monochrome art, no assets, no network
- Pure rules engine (`engine.ts`): deck, deal, turn state machine, yaku evaluation, and the CPU opponent — no React imports, unit-testable
...
- Rules knobs (round count, yaku point values, CPU heuristics) live in `engine.ts` constants — mirror any change into `HanafudaEngine.swift` / `HanafudaEngine.kt` to keep the ports in lockstep (both carry "must stay in sync with the web" doc comments)
...
- Multiplayer — the opponent is always the CPU
