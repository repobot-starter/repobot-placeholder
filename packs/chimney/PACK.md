...
Client-only vertical pack: Chimney, a night-time rooftop runner (canvas game loop, variable-height jumps, localStorage best run) at `web/app/src/View/Games/Chimney/`.

## What ships

- Monochrome game page with run-log / controls / house-rules panels (owns `/` when this pack is active; otherwise preview at `/chimney`)
