...
- One derived template rides the same pages with its own seed (see `packs/README.md` "Derived templates"): `fund-index-stet` (Stet, a two-partner pre-seed fund in New York) on the `galleyproof` register (`proofmark`), whose `home.line` switches the home to one line — the thesis alone on the first screen at poster scale with its full stop in proof red, the facts as a mono slug above it and the pitch beneath, the portfolio as a comma-run of names, the deck ask as one closing line — and drops the achromatic style pin on every page; the inner pages are otherwise unchanged
