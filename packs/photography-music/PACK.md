...
`home.layout` in `content.ts` picks the home page's shape; the work, about and book pages are the same for both.

- `"reel"` (the default, Vic Mercer) — the masthead hero, the reel, collection covers, introduction and booking CTA described above.
- `"records"` — the record wall (`docs/landing.md` "The portrait-stack set"): the first `heroSlides` frame full-bleed, then `home.records.covers` as gallery `covers` (square sleeves, each cover's `artist` over its album `title`, lightboxed; keep the count a multiple of four), then cta-banner `colophon` with `home.records.closing` (`line` and the `cta` text link to `/book`).

The `photography-music-theo` remix (Theo Marsh, album art and press photography in Nashville, `liner` register) rides the record wall from `theoRemix.content.ts`, with images in `web/app/public/photography-music-theo/`. Every artist and album name on its wall is invented.
