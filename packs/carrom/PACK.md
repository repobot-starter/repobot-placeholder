...
Client-only vertical pack: carrom, the South Asian flicking board game (canvas physics loop, WebAudio clicks, CPU opponent) at `web/app/src/View/Games/Carrom/`.

## What ships

- Monochrome game page with mode (vs the CPU / hotseat 2P), difficulty, coin trays, match score and turn/foul ticker (owns `/` when this pack is active; otherwise preview at `/carrom`)
...
- Native ports of the game as the home surface of the iOS app (`ios/App/View/Games/Carrom/`) and the Android app (`android/.../view/games/carrom/`) — drag-to-flick vs the CPU, no backend
...
- CPU skill lives in the `BOT_LEVELS` gaussian-error table in `engine.ts`.
...
- Online multiplayer — the shipped game is fully client-side (CPU or hotseat)
