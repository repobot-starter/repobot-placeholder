...
- **Content-driven sections** — the home page's middle renders only what
  its content fills: `metrics`, `included` and `gallery` (filled here), the
  `thread` conversation with the house manager (steps `message-thread`),
  the `season` calendar (schedule `week-grid`, the current window marked),
  the `specimens` board, the `turnover` rail and the `roomReady` door-hanger
  checklist (empty here), and the plans page's `addOns`. Two frames are
  content-picked too: `home.layout` sets the hero (`split`, `full-bleed`,
  `masthead`), and plans that all carry a `stub` print as season tickets;
  `home.badge` replaces the live open badge with a sticker, and
  `landingCopy` carries every trade-specific heading and banner
- **Seven derived templates** (`remixOf: services-recurring`): the originals
  `services-lawncare` (Timberline Lawn Co., deep emerald) and
  `services-pest` (Basalt Pest Control, brick red) inherit these pages on
  warm-boutique; the restyles pin their own registers and skeletons —
  `services-recurring-turnover` (Clean Getaway, Miami, `memphis`: trading-card
  prices, the turnover rail, the door-hanger checklist),
  `services-lawncare-crew` (Fresh Cut, Atlanta, `gameday`: a full-bleed
  crew photograph over season tickets), `services-pest-creature` (Night
  Crawlers, Houston, `creature`: a masthead poster and a creature index),
  `services-recurring-linen` (Linen & Laurel, Beverly Hills, `whiteglove`:
  the house-manager text thread) and `services-recurring-worth` (The Worth
  Pool Co., Palm Beach, `palmbeach`: the season calendar in a bamboo frame)
