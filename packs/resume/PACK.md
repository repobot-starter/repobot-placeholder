...
- **Derived templates** (`remixOf: "resume"`): `resume-dev` (a staff engineer, `monolith` true-black mono) and `resume-yara` (Yara Haddad, a chef de cuisine, `galley`). A seed that photographs its person turns the page photographic (`ResumePhoto` in `resumeLanding.ts`): `person.portrait` makes the hero `full-bleed-media`, a role `image` hangs the experience on a photographic steps `timeline`, a project `image` sets the work as showcase `stories`; print drops the photographs and returns the hero to ink
