...
- Recompose the home: `home.layout` picks `marquee` (masthead type over a crossfade, the person before the work, tilted prints) or `stack` (a full-bleed hero, then `home.stack` frames edge to edge with each couple's names over the photograph, closing on the `home.closing` photograph band; `home.locations` adds a spec sheet of the places). `home.wall` picks the selected-work wall and `home.albumWall` every wedding page's wall (`justified`, the roll as a `contact-sheet` with each frame's grease-pencil `mark`/`note`, or the captioned `stack`).
- Retrade the words: every heading, button label and closing line the pages print lives in `landingCopy` — the nav's variant and ask, the package currency (`landingCopy.currency`, ISO 4217: "eur" prints "€9,000"), and each page's banners. An empty heading or button drops.
...
A derived template (packs/README.md "Derived templates") ships this pack's pages with its own content seed, register, and pinned page skeletons:

- `wedding-fiorella` — Fiorella Conti, film weddings on the Amalfi Coast, Lake Como and in Puglia: the `limone` register, the `stack` home, contact-sheet walls, collections in euros (`web/app/src/View/Wedding/fiorellaRemix.content.ts`).
- `wedding-highline` — Highline, adventure elopements out of Telluride: the `alpine` register, the `stack` home of places with `home.locations` as the spec sheet, a filmstrip, every elopement as a captioned stack (`web/app/src/View/Wedding/highlineRemix.content.ts`).
