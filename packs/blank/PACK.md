...
- The spaceboy scene at `/` (`web/app/src/View/Blank/`): an animated night sky (twinkling stars, a glowing textured moon with a gentle halo pulse), a grassy hill, and the boy silhouette holding a dotted string up to the moon. Art-directed custom view, not a `LandingConfig` composition; animations respect `prefers-reduced-motion`. `/` is the scene itself — not an in-page editable title field. Site title and copy are authored through the agent (and landing/project manifests) when the placeholder is replaced.
- No domain GraphQL contract beyond the kernel exemplars (`/login`, `/users`, `/projects`)

## Agent recipe

Treat this as a blank canvas. The spaceboy scene is a placeholder with personality, not the product: keep it until the product has a real home, then replace `/` with the app shell the user asked for. Do not swap the scene for a click-to-edit title seed. Use Identity/Project exemplars as pattern references, not as the product UI.
