...
- **Payments**: BUILT — the payments kernel (`docs/payments.md`) now ships
  catalog-agnostic checkout sessions keyed by `productKey`, the Stripe
  webhook, the `purchases` ledger, and session-gated delivery, with shop as
  the consumer exemplar. Subscriptions (for ClubBot) remain the piece that
  does not exist yet and should be built once, in the kernel, not per-pack.
...
## Staged extractions — do these when the consuming pack starts

Each of these has exactly one consumer today, so extracting now would be
speculation; the second consumer's first task is the extraction. Staged here
so the work is scoped before the pack begins:

- **Hours engine → shared util, when BookBot starts.** Menu's weekly-hours
  engine (`web/app/src/View/Menu/hours.ts`, mirrored natively with tests like
  `MenuHoursTest.kt`) becomes a shared scheduling util; BookBot's slot
  generation (hours minus existing bookings) builds on it rather than forking
  it.
- **Markdown renderer → shared util, when CourseBot starts.** Blog's parser
  (`web/app/src/View/Blog/markdown.ts`) serves CourseBot lesson bodies as
  the second consumer; extract it to a shared module then, keeping Blog's
  rendering behavior pixel-identical.

Deliberately never extracted: the one-typed-content-file shape stays a
convention (mirroring pain is tooling work, not a runtime kernel), game rules
engines stay pack-local, and share buttons stay inline.
...
- Delivery: full-resolution files live outside the public bundle and are
  served by the payments kernel's session-gated delivery endpoint
  (`GET /delivery` verifies the session is PAID server-side; files live in
  `firebase/functions/delivery/<productKey>/`) — the "content unlocked by
  purchase" primitive already shipped with the kernel; GalleryBot is its
  first multi-product consumer and CourseBot reuses it for paid lessons.
...
- Stripe: one-off checkout per tip via the payments kernel; the kernel's
  webhook + `purchases` ledger already record tips, so the page's honest,
  owner-approved "recent supporters" list is a query away.
- Simplest Wave 2 pack — ship first to prove one-off payments outside shop.
  Receipts come free: the payments kernel already emails one per ledgered
  purchase through the mail kernel (`Services/Mail/`, see `docs/mail.md`).
