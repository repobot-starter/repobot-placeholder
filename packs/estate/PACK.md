...
- One derived template rides the same pages with its own seed (see `packs/README.md` "Derived templates"): `estate-aldercott` (Aldercott, a Bellingham waterfront broker) on the `baylight` register (`framestack`, `soundings`), whose `home.stack` switches the home to the plates — the featured listings full-bleed at one size, each captioned inside its photograph with its place, price, and computed status; the inner pages are unchanged
