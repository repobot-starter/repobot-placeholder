...
Client-only vertical pack: a link-in-bio page (Link) at `web/app/src/View/Link/`.

## What ships

- A single-column bio page: avatar, name/handle/bio, social monogram chips, link rows with hover lift, a share button (Web Share API with clipboard fallback), and a "Made with Link" footer (owns `/` when this pack is active; otherwise preview at `/link`)
