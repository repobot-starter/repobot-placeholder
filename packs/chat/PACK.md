...
- Modes mirror auth and payments: in the workspace `AI_MODE=local` streams a
  simulated assistant (no key, no cost, identical protocol); deploys run the
  real model automatically — `AI_MODE=gateway` through the platform's AI
  gateway (keyless, billed to the account's platform credits) or
  `AI_MODE=openai` when the account has its own OpenAI integration
  connected. Users never need to bring an API key; never tell them to.
...
- Real model calls in the workspace (the simulated assistant is by design; set
  `AI_MODE=openai` plus `OPENAI_API_KEY` in `firebase/functions/.env.local` to
  test the real model locally — deploys need no key, the platform's AI
  gateway covers keyless accounts)
