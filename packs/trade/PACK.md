...
- An editorial paper-and-ink page — KPI stat strip, commodity cards with spec chips, a supply-chain journey timeline, the shipment board, certifications, and a partner wall — composed on the landing kernel (`editorial` preset; the `LandingConfig` in `tradeLanding.ts` maps `content.ts` into sections, `docs/landing.md`); owns `/` when this pack is active, otherwise preview at `/trade`
...
- Shipment rows carry a `tone` (`success` / `info` / `warning` / `neutral`); the web page now shows the status as a tag chip (the landing vocabulary has no toned pills), while the native ports keep the toned palette.
