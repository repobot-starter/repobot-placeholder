...
Client-only vertical pack: the classic paddle duel (canvas game loop, WebAudio bleeps, CPU opponent) at `web/app/src/View/Games/Pong/`.

## What ships

- Monochrome game page with mode/difficulty/speed controls (owns `/` when this pack is active; otherwise preview at `/pong`)
...
- Native ports of the game as the home surface of the iOS app (`ios/App/View/Games/Pong/`) and the Android app (`android/.../view/games/pong/`) — touch paddle vs the CPU, no backend
