...
Client-only vertical pack: a paint studio (stacked paper/overlay canvases, flood fill, shape previews, emoji stickers) at `web/app/src/View/Games/Paint/`.

## What ships

- Studio page with tool rail, stroke settings, sticker tray, and swatch palette (owns `/` when this pack is active; otherwise preview at `/paint`)
