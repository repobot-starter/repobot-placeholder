...
Client-only vertical pack: a three-lane night highway racer (canvas game loop, lane-change steering, nitro boost, overtake scoring) at `web/app/src/View/Games/Race/`.

## What ships

- Monochrome game page with telemetry/controls/race-brief panels and a localStorage best run (owns `/` when this pack is active; otherwise preview at `/race`)
...
- Canvas renderer with scrolling rumble strips, dashed lane dividers, monochrome cars, and a nitro flame — no assets, no network, no backend
