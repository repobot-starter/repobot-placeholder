...
3. Dataloaders batch through the owning service's batch method: `orderedBatchLoad` + `orderedBatchLoad<X>sByIds` for to-one relations, `orderedGroupLoad` + `orderedBatchLoad<X>sBy<ParentId>s` for one-to-many lists (exemplar: `Project.memberships` through `projectMembershipsDataloader`; a parent with no children gets `[]`). Both helpers live in `firebase/functions/src/Utils/DataLoaderUtils.ts`.

This keeps connections N+1-free. Add a new dataloader to `GraphqlRequestContext` when another domain starts referencing your entity, and extend `firebase/functions/test/Project/DataloaderBatchingTest.ts`'s pattern for it: spy the service batch method, resolve a page, assert one grouped call and zero per-row calls.
