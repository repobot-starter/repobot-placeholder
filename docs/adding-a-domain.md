...
The Projects exemplar already demonstrates the full list-page kit — copy its wiring, don't rebuild it: facet filters via the view model's `filters` (mapped to connection filter inputs), column-header sorting via `tableSort`/`onTableSortChange` (column ids mapped to the service's `sortColumnKeys`), and success/failure toasts via `useToast`. For workflow entities (orders, tickets, invoices) the status facet + status `Badge` column + row actions pattern is exactly what to clone; money renders as formatted text in a column (store cents in SQL, format in the column render). For dashboard-style destinations, compose `StatCard`, `ChartCard`, and `ActivityFeed` over the same queries (see `docs/design-system.md`).
