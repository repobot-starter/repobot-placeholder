...
| 4   | `feature-grid`  | "What does it do for me?" (many parallel features)        | **`cards-3up`**, **`icon-list`** (compact 2-col), `bento` (asymmetric grid with one flagship cell), `checklist` (door-hanger card, check states)                                                                                   | kicker, title, features[] {emoji/icon, title, description, checked?}                              |
...
| 6   | `steps`         | "How does it work? Is it hard?"                           | **`numbered-cards`** (launch-style), `timeline` (vertical, accent-per-step), `horizontal-rail` (photo cards on one connecting line, optional per-step `label` + `media`)                                                           | kicker, title, steps[] {title, description, label?, media?}                                       |
...
| 9   | `pricing`       | "What does it cost? Which plan is me?"                    | **`tiers`** (2–3 cards, one highlighted, optional monthly/yearly toggle — launch-proven), `single-price`, `table` (feature-by-tier matrix); a tier may carry `pricePrefix` ("From")                                                | tiers[] {name, monthly, yearlyPerMonth, description, features[], highlighted?, badge?}            |
| 10  | `faq`           | Residual objections                                       | **`accordion`** (native `<details>`, launch-proven), `two-column`                                                                                                                                                                  | items[] {question, answer}                                                                        |
| 11  | `showcase`      | "Show me the work / the goods" (portfolio, menu, gallery) | **`card-grid`**, `filterable-grid` (tag chips derived from item tags, folio-proven), `media-rail`, `specimens` (tall portrait plates; `meta` as the "/ use"), `swatches` (flat color chips; item `color`)                          | items[] {title, description, tags?, media/emoji+accent, url?}                                     |
...
- `hero` `form-first` composes the `lead-form` content shape rather than
  duplicating it. The capture may grow into an intake card
  (`MarketingLeadIntake`): a `heading`, single-choice `choices`
  (`{label, icon?}`, submitted as `choice`), one extra short `field`
  (`{name, label, placeholder?}`, submitted under its name) and a `contact`
  of `email` (default) or `tel` (submitted as `phone`, with no email key).
  An intake with a `heading` bound to an image `media` renders as the
  photograph's own tool — the frame full-bleed, the copy white on the
  scrim, the card at the right — the dispatcher's first question over the
  scene it's about.
...
Registers may also carry optional extras, per mode: `spot` (two spot inks,
emitted as `--marketing-spot-1`/`--marketing-spot-2`, falling back to the
accent), `ornament` (an SVG tile emitted as `--marketing-ornament`, painted
behind the page by the `confetti` treatment and hidden below 1320px so it
never crowds copy), and treatment flags on the page root
(`data-marketing-treatment`) that sections opt into per style — e.g. `pop`
(`memphis`: ink-tape kickers, italic titles with a spot-ink offset, sticker
photos, trading-card tiers, the starburst hero badge), `two-ink` (`riso`:
photographs reprinted as a two-ink plate pair through a shared SVG filter,
the inset-print hero with a caption stamp, the spot-ink ticker band,
brush-stroked titles), `colorblock` (`paintchip`: the edge-to-edge split
hero, accent-square kickers, hard-shadowed square CTAs), `metallic`
(`crown`: chrome and gold display type cast as real text through a
gradient clip, gold-ruled kickers behind a sparkle, the hero's cover lines,
the static price strip) and `lacquer` (`vanity`: Didone heads over tiny
spaced caps, blush hairlines, glossed square CTAs, shade cards riding over
the hero's foot). A section never matches a preset by name.

More treatment flags in the same grammar: `sport` (`gameday`: volt-tape
kickers, condensed italic caps, slanted CTAs with a forward arrow, a
shorter full-bleed hero so the next row reads in the first view, the
accent-plate closing banner) and `pulp` (`creature`: poster caps extruded
in the accent, spot-ink billing tabs, a starburst masthead badge, the
credit as a full-width billing block, lobby-card mounts on tiers,
specimens, and rail frames). Two more carry family-scale registers:
`taped` (`snapshot`: white-bordered prints under masking tape at small
leans across the pinboard hero, scrapbook, collections, tiers, rail, and
closing banner, marker-script captions and times, the highlighted tier's
badge as a round sticker) and `lariat` (`rodeo`: a rope frame around the
whole page drawn from the register's `ornament` tile as a border image,
wood-type program rows, brand-tag stats, numbered claim-ticket cards, a
rope-framed checklist, star bullets and rail stops).
Three quiet portfolio registers carry no treatment beyond hairlines —
`vitrine` (stone grey, small EB Garamond), `seamless` (white, small Inter
Tight) and `liner` (near-black sleeve, small Barlow) — and lean on the
portrait-stack variants instead: gallery `sequence` + `captionStack`
`band-*` (one frame size for the whole stack, cover-cropped, caption bands), gallery
`covers` (square record sleeves, artist over title) and cta-banner
`colophon` (one line and a text link).
Three quiet wedding registers ride the vows pack's `story`, `stack`, and
`letter` homes — `seafog` (grey-blue coastal mist, a light Cormorant with a
small Jost), `harvest` (forest-green stock and ivory ink, a light
Newsreader; dark-native), and `carton` (cornflower ink on warm paper, an
enormous hairline Bodoni Moda). Only `carton` adds a treatment beyond
hairlines: `ruled`, the faire-part — a statement hero carrying a badge
sets its names enormous over a tracked-caps line under an ink rule
(page-title statements keep the register's size), and rich-prose `narrow`
runs flush to the hero's edge as one column divided by ink hairlines.

Treatments that restyle many sections at once keep their rules in one
sheet per flag under `web/design-system/src/marketing/registers/`
(`globalStyle` against the components' exported classes, keyed on the page
root's flag): `transit` (`wayfinding`: the sign-band nav, the split hero as
a full-bleed station sign panel, route-bar kickers, circular pictograms,
heavy rule lines, arrowed CTAs), `sleeve` (`groove`: the LP front — square
cover, the record sliding out, a label-sticker seal, a three-stripe band —
and A1/B1 tracklists for the hero directory and the price list),
`wall-label` (`inkblot`: the title across the wall, the plate multiplied
into it, the directory as the museum label, bare numbered plates, the
exhibition price list), and `candy` (`bubblegum`: gradient-filled display
caps on a drop-shadow "candy drop", die-cut sticker directory, a heart
seal, glossy pills, speech-bubble quotes).

The quiet-practice registers keep the same one-sheet rule: `gilt`
(`parlor`: kickers engraved between gilt rules, the full-bleed hero graded
warm, the photographic timeline hung on one gilt line with inset-matted
photographs, engraved promise and price cards, arched portraits, the
ornament as a magnolia sprig over the footer imprint) and `fieldbook`
(`trailhead`: left-set headers under a pencil-trail kicker, the full-bleed
hero graded pine, inked icon discs on the promise cards, the `stories`
showcase re-gridded into field-note rows — photograph, pencil map, essay —
with the column rule's pseudo-element repurposed as the map, one cell of
the ornament strip per note, and a contour band over the footer) and
`weave` (`hearth`: the ornament — one woven diamond tile — laid as a
vertical edge down the full-bleed hero, a horizontal one along its foot and
over a plum footer; the `filterable-grid` showcase as a portrait roster
with pill filters and ochre tag chips; numbered steps on a forest band and
testimonials on a sienna band, both bled to the viewport from inside the
column) and `marginalia` (`colophon`: kickers in the preset's `fonts.script`
small capitals under the ornament — a printer's star — the `report`
content split re-gridded with its copy column dissolved (`display:
contents`) so the kicker and headline span the spread over the paper case
file and the bullets become counter-numbered margin notes with hairline
leaders; numbered steps as ruled columns whose digits give way to
counter-drawn roman numerals; a double rule over the footer with the star
set into it) and `frost` (`boreal`: the full-bleed hero's scrim replaced by
a light frost wash with the copy re-inked slate over it; the
`horizontal-rail` drawn as one track whose last node and final stretch take
spot 1, re-flowed at phone width from a sideways scroller into a vertical
line by re-pointing each item's connecting pseudo-element; the
photograph-right content split's bullets as an inline run, each word but
the last trailed by a spot-1 point).
