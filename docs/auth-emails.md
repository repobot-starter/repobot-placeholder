...
rejects the file (and keeps the previous templates). Which variable matters
depends on the email type:

- `magicLink` and `recovery`: the app's UI asks the user to type the code,
  so keep `{{ .Token }}` prominent (the link is a fallback).
- `confirmation`: the app has **no** code-entry step after sign-up — the
  user's only path is clicking `{{ .ConfirmationURL }}`, so render it as a
  prominent button (as the default does) and leave the code out entirely.
  Clicking the link verifies the email and signs the user in.

Two more variables are available but optional:

- `{{ .AppName }}` — the app's display name (`APP_NAME`, falling back to the
  deployed hostname); the defaults use it as the wordmark
- `{{ .SiteURL }}` — the environment's live URL (see images below)
...
- **Deploy time only (this file).** The platform applies the file at the
  deployed branch after each successful deploy. There is no live sync while
  editing. Templates saved on the dashboard's Email Templates page are
  different: they apply to deployed environments within about a minute and
  win over this file per email type.
