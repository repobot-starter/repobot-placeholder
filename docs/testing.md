...
Template-shipped test files under `web/app/tests/` are pinned: a hash manifest
(`web/app/tests/.pinned-tests.json`, verified by every check run) fails the
gate if one is modified or deleted. Those tests encode the kernel's promises —
the seed contract in `blueprints.test.ts`, SEO defaults, brand wiring — so
when one fails after a change, fix the code so the promise still holds. New
test files are welcome and are not pinned. (Kernel developers refresh the
manifest with `node scripts/verify-pinned-tests.mjs --write` after intentional
template test changes.)

## Never

- Never call services or the database directly when a GraphQL path exists.
- Never weaken, rewrite, or delete a template-shipped test (or edit the pinned-tests manifest) to get a gate green — change the code so the pinned promise still holds, and add new tests for new behavior.
